...
This canonical planning addendum preserves implementation candidates, mechanisms, rejected alternatives, and evidence-spike detail that should not appear as normative product requirements. It accompanies `PRD.md`; it is not architecture approval and does not authorize product implementation. Mandatory engineering-process rules in Sections 9 and 10 are normative only through `docs/PROJECT-GOVERNANCE.md` and `docs/QUALITY-GATES.md`, which take precedence over this addendum.
