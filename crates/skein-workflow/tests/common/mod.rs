...
//!
//! The shape is `crates/skein-core/tests/native_loop.rs`'s, deliberately: a
...
use serde_json::json;
use skein_core::{
    Ledger, Message, ModelClient, Redactor, Result, SkeinError, Step, StepKind, ToolAccess,
...
/// which `skein-core`'s own suite already covers at the layer that owns it.
pub fn engine(
    model: ScriptedModel,
