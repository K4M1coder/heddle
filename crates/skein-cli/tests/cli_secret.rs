//! Acceptance tests for `skein secret set|delete` (spec 011).
//!
//! Against the **real** platform credential store, driving the **real** binary,
//! following `crates/skein-silo/tests/silo_secret.rs`'s conventions: test
...
use skein_core::{SecretProvider, SecretRef};
...
fn skein_with_stdin(args: &[&str], stdin: &[u8]) -> Output {
...
fn e8_secret_set_then_delete_round_trips_without_printing_the_value() {
    ...
    let set = skein_with_stdin(&["secret", "set", held.uri()], b"hunter2\n");
    ...
    let deleted = skein_with_stdin(&["secret", "delete", held.uri()], b"");
...
fn e9_secret_set_has_no_value_flag_and_refuses_an_empty_secret() {
    ...
    let flagged = skein_with_stdin(&["secret", "set", held.uri(), "--value", VALUE], b"");
    assert_eq!(
        flagged.status.code(),
        Some(2),
    ...
    let empty = skein_with_stdin(&["secret", "set", held.uri()], b"");
