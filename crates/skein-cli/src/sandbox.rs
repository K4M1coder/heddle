//! `skein sandbox list|prune` — a rendering of `skein-sandbox`'s two cleanup
...
//! take it back; the proof that a removal removes only what skein granted is
//! structural and lives in the library.

use skein_core::{Result, SkeinError};
use skein_sandbox::{Grant, GrantKind, GrantState, Pruned};
...
/// ```
pub fn list() -> Result<()> {
    for grant in skein_sandbox::grants().map_err(refused)? {
...
/// One named profile, or every profile skein made. clap makes the choice
...
pub fn prune(profile: Option<&str>, all: bool) -> Result<()> {
    ...
    for name in selected {
        report(skein_sandbox::prune(&name).map_err(refused)?);
    }
    Ok(())
}
...
/// `skein-sandbox` is a leaf that depends on no Skein crate, so it refuses with
/// a `String` and the mapping happens here.
///
/// `Storage` because this is durable local machine state that could not be read
/// or removed — the nearest thing `SkeinError` has. A `Sandbox` variant would
/// read better and is deliberately not added: it would mean editing
/// `skein-core` for a rendering concern at the outermost layer.
fn refused(reason: String) -> SkeinError {
    SkeinError::Storage(reason)
}
