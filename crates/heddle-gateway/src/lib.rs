...
//! `heddle-mcp` is the only one naming MCP and `heddle-acp` the only one naming
//! ACP. `heddle-core` does not depend on it — it discovers a provider through
...
//!
//! The client is synchronous, matching `ModelClient::turn`. `heddle-acp` calls
...
use heddle_core::{
    HeddleError, Message, ModelClient, Result, SecretValue, TextSink, ToolCall, ToolSpec,
        ...
        match uri.scheme_str() {
            ...
            Some(scheme) => {
                return Err(refuse(format!(
                    "scheme {scheme:?} is refused; Heddle v0 talks to local providers over http \
...
pub struct OpenAiCompatClient {
    ...
    /// caller with nothing to show before the turn ends — `heddle chat`, whose
...
impl OpenAiCompatClient {
    ...
    fn send(&mut self, url: &str, body: &str) -> Result<(u16, Answer)> {
        ...
        if !(200..300).contains(&status) {
            let raw = response.body_mut().read_to_string().map_err(|e| {
                HeddleError::Model(format!("POST {url} returned an unreadable body: {e}"))
            })?;
            return Ok((
                status,
                Answer {
    ...
    fn unrecognised(&self, detail: impl std::fmt::Display) -> HeddleError {
        HeddleError::Model(format!(
...
fn drain(mut reader: impl BufRead, sink: &mut Option<Box<dyn TextSink>>) -> Answer {
    ...
    loop {
        ...
        // sink means nobody to cancel, which is `heddle chat`.
    ...
    fn from(message: &'a Message) -> ChatMessage<'a> {
        ChatMessage {
            role: match message.role {
                heddle_core::Role::User => "user",
                heddle_core::Role::Assistant => "assistant",
                heddle_core::Role::System => "system",
                heddle_core::Role::Tool => "tool",
            },
