...
use heddle_core::{
    Exit, HeddleError, Ledger, LoopBudget, LoopController, Message, NativeLoop, ProgressProbe,
    Redactor, Result, StepKind, ToolCall, ToolGateway, ToolOutcome, ToolPolicy, ToolTransport,
    WireExchange,
};
...
/// `heddle chat` supplies: no ground truth, and a transport no name can reach.
...
fn a_provider_failure_ends_the_run_with_the_request_already_on_the_chain() {
    ...
    assert!(matches!(error, HeddleError::Model(_)), "got {error:?}");
