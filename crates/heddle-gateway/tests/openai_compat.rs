...
use heddle_core::{
    Content, HeddleError, Message, ModelClient, Role, TextSink, ToolSpec, TurnRequest,
};
    ...
    model
        .turn(&TurnRequest {
            ...
            tools: vec![ToolSpec::new(
                ...
                // `BTreeMap` under `cargo test -p heddle-gateway`. Writing the
...
fn a_conversation_history_is_sent_in_order() {
    ...
    model
        .turn(&ask(vec![
            ...
            Message::assistant_text("").with_tool_calls(vec![heddle_core::ToolCall::with_id(
                "call_1",
                "fs_read",
...
fn refusal(base_url: &str) -> String {
    match LocalEndpoint::parse(base_url) {
        ...
        Err(HeddleError::Model(message)) => message,
        Err(other) => panic!("expected HeddleError::Model, got {other:?}"),
    }
}
...
/// `HeddleError::Model` message, so a test asserts *why* it refused.
...
fn expected_calls() -> Vec<heddle_core::ToolCall> {
    vec![
        heddle_core::ToolCall::with_id(
            "call_43lp106j",
            "fs_read",
...
fn tool_calls_fragmented_across_deltas_accumulate_to_the_same_calls() {
    ...
    // never sends it — but `heddle-cli`'s wiring documents a LiteLLM sidecar as
...
fn a_tool_call_with_no_argument_fragments_is_read_as_an_empty_object() {
    ...
    assert_eq!(
        response.tool_calls,
        vec![heddle_core::ToolCall::with_id(
            "call_1",
            "git_status",
            serde_json::json!({})
        )]
...
fn cancelled_turn(stub: &Stub, sink: Box<dyn TextSink>) -> (String, heddle_core::WireExchange) {
    let mut model = client(stub.base_url(), "llama3.1");
...
/// ```
#[test]
#[ignore = "needs a real local provider; set HEDDLE_LIVE_MODEL to run"]
fn a_live_local_provider_stops_when_the_sink_stops_wanting_text() {
    let Some(model_name) = std::env::var_os("HEDDLE_LIVE_MODEL") else {
        eprintln!("HEDDLE_LIVE_MODEL is unset; skipping the live provider test");
        return;
    };
...
///
/// ```text
/// $env:HEDDLE_LIVE_MODEL = "llama3.1"
/// cargo test -p heddle-gateway --test openai_compat -- --ignored --nocapture
/// ```
#[test]
#[ignore = "needs a real local provider; set HEDDLE_LIVE_MODEL to run"]
...
///
/// ```text
/// $env:HEDDLE_LIVE_MODEL = "qwen3.8:27b"
/// cargo test -p heddle-gateway --test openai_compat -- --ignored --nocapture
/// ```
#[test]
