...
/// has never run skein, so an operator could not tell "this platform makes no
/// profiles" from "you have none". Fail clearly, never silently degrade.
#[test]
fn grants_and_prune_refuse_off_windows() {
    let unlistable = skein_sandbox::grants().expect_err("no platform but Windows has profiles");
    let unprunable = skein_sandbox::prune("skein-0000000000000000")
