...
use heddle_core::{
    replay_tool_calls, HeddleError, Ledger, LoopBudget, LoopController, Message, ModelClient,
    NativeLoop, ProgressProbe, Redactor, Role, StepKind, ToolAccess, ToolCall, ToolGateway,
...
impl ServerHandler for DownstreamServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            .with_instructions("heddle-mcp test downstream")
    }
}
...
fn c1_policy_denial_never_reaches_the_mcp_server() {
    ...
    assert!(matches!(err, HeddleError::ToolDenied { .. }), "got {err:?}");
    assert_eq!(
...
impl ModelClient for ScriptedModel {
    fn turn(&mut self, _req: &TurnRequest) -> heddle_core::Result<TurnResponse> {
        let i = self.calls;
        self.calls += 1;
