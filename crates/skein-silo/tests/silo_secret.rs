...
//! that Skein can read the vault the operator already trusts.
//!
...
use serde_json::json;
use skein_core::{
    Ledger, Redactor, Result, SecretProvider, SecretRef, SkeinError, StepKind, ToolAccess,
...
fn k2_a_missing_reference_fails_loudly() {
    ...
    let never_stored = SecretRef(format!(
        "keychain://skein-test-{}-never/redactor",
        std::process::id()
    ));
