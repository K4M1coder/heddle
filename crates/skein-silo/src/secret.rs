...
//! `skein-core` discovers secrets through `SecretProvider` and never through an
...
use skein_core::{Result, SecretProvider, SecretRef, SecretValue, SkeinError};
use std::sync::Arc;
...
fn parse(secret: &SecretRef) -> Result<(&str, &str)> {
    let refuse = |why: &str| SkeinError::Secret(format!("secret reference {:?} {why}", secret.0));
    let rest = secret
        .0
        .strip_prefix(SCHEME)
...
fn refused(secret: &SecretRef, err: Error) -> SkeinError {
    SkeinError::Secret(format!("{}: {err}", secret.0))
}
...
fn unopened(err: Error) -> SkeinError {
    SkeinError::Secret(format!("the platform credential store did not open: {err}"))
}
