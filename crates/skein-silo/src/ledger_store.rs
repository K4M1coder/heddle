...
//!
//! `skein-core` reaches durable storage through `LedgerStore` and never learns
//! what is behind it (Constitution IV), exactly as it reaches MCP through
//! `ToolTransport` and ACP through nothing at all.

use rusqlite::{params, Connection};
use skein_core::{LedgerStore, Result, SkeinError, Step, StepKind};
use std::path::Path;
...
fn seq_to_sql(seq: u64) -> Result<i64> {
    i64::try_from(seq).map_err(|_| SkeinError::Storage(format!("step seq {seq} exceeds i64")))
}

fn seq_from_sql(seq: i64) -> Result<u64> {
    u64::try_from(seq).map_err(|_| SkeinError::Storage(format!("stored seq {seq} is negative")))
}

fn storage(error: rusqlite::Error) -> SkeinError {
