...
//! `heddle-mcp` names it as a client, and the two meet over an in-process duplex
...
/// discovers it over `tools/list` and `heddle-gateway` puts it on the wire. A
...
impl EmbeddedServer {
    ...
    /// to `HeddleError::Tool`, and `NativeLoop::mediate` treats as fatal — so
    /// `heddle-cli`'s allowlist must omit the same two names in the same case,
    ...
    /// as a protocol error, `RmcpToolTransport` maps to `HeddleError::Tool`, and
    /// `NativeLoop::mediate` treats as fatal. So `heddle-cli`'s allowlist must
    ...
    pub fn with_run(
        ...
        run: RunAccess,
        cancelled: Arc<AtomicBool>,
    ) -> heddle_core::Result<Self> {
        let launcher = match run {
            RunAccess::Denied => None,
            RunAccess::Allowed(dirs) => Some(Launcher {
                sandbox: Arc::new(
                    Sandbox::create(root.path(), dirs.paths()).map_err(HeddleError::Tool)?,
                ),
                cancelled,
            }),
        };
...
impl ServerHandler for EmbeddedServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions(
            "Heddle's embedded connector over one operator-named root. Every filesystem path is \
