...
//!
//! **This is the only module in the workspace that names `heddle-sandbox`,** the
...
use heddle_sandbox::{Captured, Run, Sandbox};
...
pub(crate) fn named(path: &Path) -> String {
    ...
    match rendered.strip_prefix(VERBATIM) {
        ...
        // `UNC` as the volume — the same case `heddle-sandbox`'s own renderer
