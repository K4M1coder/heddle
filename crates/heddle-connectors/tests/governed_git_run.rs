...
//! reason that file restates `heddle chat`'s policy rather than importing it:
//! `heddle-cli` has no `lib` target and Rust integration-test binaries do not
//! share helpers.
//!
...
/// `heddle chat`'s policy over a root that **is** a repository, restated here
/// rather than imported: `heddle-cli` has no `lib` target.
///
...
/// cargo test -p heddle-connectors --test governed_git_run -- --ignored --nocapture
/// ```
///
