...
use heddle_connectors::{local_connector_with_run, FsRoot, RunAccess, RunDirs};
use heddle_core::{
...
/// `governed_fs_run.rs` records: `heddle-cli` has no `lib` target to share it
/// through.
...
///
/// **`$HEDDLE_LIVE_RUN_DIR` grants that directory a read-and-execute ACE that
...
/// `ERROR_ACCESS_DENIED`.
///
/// With `$HEDDLE_LIVE_SILO_ROOT` set the chain is persisted to a real silo, so
/// `heddle ledger log` and `heddle ledger show` can read the evidence back **in a
...
/// ```text
/// $env:HEDDLE_LIVE_MODEL     = "qwen3:8b"
/// $env:HEDDLE_LIVE_RUN_DIR   = "D:\Users\you\.rustup\toolchains\1.97-x86_64-pc-windows-msvc\bin"
/// $env:HEDDLE_LIVE_SILO_ROOT = "$env:TEMP\heddle-live-020"
/// cargo test -p heddle-connectors --test governed_proc_run -- --ignored --nocapture
/// ```
#[test]
#[ignore = "needs a real local provider and a real toolchain directory; set HEDDLE_LIVE_MODEL and \
            HEDDLE_LIVE_RUN_DIR to run"]
fn a_live_model_runs_a_real_toolchain_binary() {
    let (Some(model_name), Some(run_dir)) = (
        std::env::var_os("HEDDLE_LIVE_MODEL"),
        std::env::var_os("HEDDLE_LIVE_RUN_DIR"),
    ) else {
        eprintln!(
            "HEDDLE_LIVE_MODEL or HEDDLE_LIVE_RUN_DIR is unset; skipping the live run-dir test"
        );
        return;
    };
    let model_name = model_name.to_string_lossy().to_string();
    ...
    // `heddle ledger show` can be the evidence rather than this test's stdout.
    let mut ledger = match std::env::var_os("HEDDLE_LIVE_SILO_ROOT") {
        Some(root) => heddle_silo::Silo::open(std::path::Path::new(&root), "live020")
            .and_then(|silo| silo.ledger())
            .expect("a silo at $HEDDLE_LIVE_SILO_ROOT"),
        None => Ledger::new(),
    };
    let mut controller = LoopController::new(LoopBudget::new(4, 1_000_000, 4));
