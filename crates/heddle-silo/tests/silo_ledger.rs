...
use heddle_core::{
    ...
    ProgressProbe, Redactor, Result, StepKind, ToolAccess, ToolCall, ToolGateway, ToolOutcome,
...
// ---- doubles ----
// Copied, not shared: the ones in `heddle-core`'s and `heddle-mcp`'s test
...
fn s3_a_write_in_one_silo_is_invisible_from_another() {
    ...
    assert!(
        matches!(beta_led.show(&alpha_id), Err(HeddleError::NotFound(_))),
...
fn s4_silo_id_cannot_escape_the_root() {
    ...
    for id in ["../outside", "..", ".", "", "a/b", "a\\b", "alpha/../.."] {
        ...
        assert!(
            matches!(err, HeddleError::Storage(_)),
...
fn s6_row_level_tampering_is_detected_on_reopen() {
    ...
    assert!(
        matches!(err, HeddleError::LedgerIntegrity { .. }),
        "got {err}"
    );
}
