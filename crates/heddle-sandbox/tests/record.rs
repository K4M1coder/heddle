...
fn a_created_profile_is_listed_with_its_directories_and_their_live_state() {
    ...
    let listed = heddle_sandbox::grants().expect("the profiles on this machine are listable");
    let mine = listed
        .iter()
    ...
    assert_eq!(root_entry.kind, heddle_sandbox::GrantKind::Root);
    assert_eq!(root_entry.state, heddle_sandbox::GrantState::Granted);

    let run_entry = dirs
        .iter()
        .find(|dir| holds(std::slice::from_ref(&dir.path), toolbin.path()))
        .unwrap_or_else(|| panic!("the run directory must be listed, got {dirs:?}"));
    assert_eq!(run_entry.kind, heddle_sandbox::GrantKind::RunDir);
    assert_eq!(run_entry.state, heddle_sandbox::GrantState::Granted);
}
...
///
/// Creates its own profile rather than assuming the machine already has one:
/// a freshly renamed prefix (or a machine that has never run a sandbox)
/// starts with none, and "not empty" must hold on its own, not by accident of
/// whatever cruft a prior run happened to leave behind.
#[test]
fn nothing_but_a_heddle_hash_name_is_listed() {
    let root = TempDir::new().expect("a temp root");
    let sandbox = Sandbox::create(root.path(), &[]).expect("the profile and the grant");
    let _pruned_sandbox = guard::PrunedOnDrop::of(&sandbox);
    ...
    assert!(
        !listed.is_empty(),
        "this test just created a profile, so an empty listing means the scan found nothing"
    );
    for grant in &listed {
        let hash = grant
            .profile
            .strip_prefix("heddle-")
            .unwrap_or_else(|| panic!("{} is not a heddle profile", grant.profile));
        assert!(
            hash.len() == 16
                && hash
                    .bytes()
                    .all(|b| b.is_ascii_digit() || (b'a'..=b'f').contains(&b)),
            "{} is not heddle- plus 16 lowercase hex characters",
            grant.profile
        );
    }
}
