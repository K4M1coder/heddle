//! Finding the AppContainer profiles Heddle made, and taking them back.
//!
...
//! ([`profile::is_heddle_profile`]) says what may be acted on, and it is the
//! same gate on both sides of the module.

use crate::profile::{is_heddle_profile, string_sid, wide, NAME_HASH_BYTES};
...
pub(crate) fn grants() -> Result<Vec<Grant>, String> {
    ...
    for entry in entries {
        ...
        let Some(name) = name.to_str().filter(|name| is_heddle_profile(name)) else {
            continue;
        };
        found.push(describe(name)?);
    }
    // Sorted so two runs of `heddle sandbox list` over an unchanged machine print
...
pub(crate) fn prune(profile: &str) -> Result<Pruned, String> {
    if !is_heddle_profile(profile) {
        ...
        return Err(format!(
            "{profile} is not a profile heddle could have created: the name must be heddle- followed by {} lowercase hexadecimal characters",
            NAME_HASH_BYTES * 2
        ));
    }
...
/// the single `PSID` derived from a `heddle-<hash>` name. The access mask and the
...
fn not_writable(reason: windows::core::Error) -> String {
    format!(
        "its permissions are not writable: {reason}; a directory's permissions are writable only          by its owner or an administrator, so run heddle elevated once or name a directory you own"
    )
}
