...
pub(crate) const RECORD_NAME: &str = "heddle-grants";
...
mod tests {
    ...
    ///
    /// Deliberately **not** a name the cleanup gate accepts — `heddle-` plus 16
    /// lowercase hex characters is what `is_heddle_profile` requires, and this
    ...
    impl Scratch {
        fn new() -> Scratch {
            let name = format!("heddle-record-test-{}", std::process::id());
