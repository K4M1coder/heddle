...
/// The prefix that separates a profile Heddle minted from the hundreds of Store
/// packages sitting beside it in `%LOCALAPPDATA%\Packages`.
const NAME_PREFIX: &str = "heddle-";
...
fn unwritable(reason: String) -> String {
    format!(
        "{reason}; a directory's permissions are writable only by its owner or an \
         administrator, so run heddle elevated once or name a directory you own"
    )
}
...
/// `heddle-` plus 16 hex characters of `sha256(root path)`.
///
...
///
/// `heddle-` plus 16 lowercase hex characters is a namespace nothing else on a
...
pub(crate) fn is_heddle_profile(name: &str) -> bool {
