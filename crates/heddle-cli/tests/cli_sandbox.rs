//! Acceptance tests for `heddle sandbox list|prune` (spec 024).
//!
...
//! directory's security descriptor is `heddle-sandbox`'s `tests/prune.rs`,
...
mod windows {
    ...
    fn a_real_grant_is_listed_and_pruned_through_the_binary() {
        ...
        let sandbox = heddle_sandbox::Sandbox::create(root.path(), &[toolbin.path().to_path_buf()])
        ...
        let listed = heddle(&["sandbox", "list"]);
        ...
        let pruned = heddle(&["sandbox", "prune", "--profile", &profile]);
