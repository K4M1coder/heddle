//! Acceptance tests for `heddle ledger log|show|verify` (spec 011).
//!
...
//!
//! Cargo sets `CARGO_BIN_EXE_heddle` for integration tests of a package with a
//! `[[bin]]`, so reaching the binary needs no test-harness dependency.

use heddle_core::{Step, StepKind};
