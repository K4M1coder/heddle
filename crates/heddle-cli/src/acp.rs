//! `heddle acp-agent` — the ACP facade slice 008 built, as a running process.
//!
//! Wiring only, like every other subcommand: the protocol and its executor are
//! `heddle-acp`'s, the model client is `heddle-gateway`'s, the loop and the budget
//! are `heddle-core`'s, and every session's chain is `heddle-silo`'s.
//!
...
use crate::SiloArgs;
use heddle_acp::{HeddleAgent, SessionParts};
use heddle_core::Result;
