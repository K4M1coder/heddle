//! `heddle` — the reference CLI client (design §4.1, Constitution I).
//!
...
//! its own: each subcommand is a call onto `heddle-core`/`heddle-silo` plus a
//! rendering of the result.
//!
//! Two commands *run* the loop rather than reading its record. `heddle chat`
//! takes one prompt and prints one answer; `heddle acp-agent` serves slice 008's
...
//!
//! `heddle acp-agent` alone takes a **second** opt-in, `--allow-run`, for a
...
/// `bin_name` is set explicitly so usage text reads `heddle …` on every OS —
/// without it clap derives `heddle.exe` on Windows from the executable name.
#[derive(Parser)]
#[command(
    name = "heddle",
    bin_name = "heddle",
    version,
    about = "Inspect a Heddle silo's ledger and provision its secrets"
)]
struct Cli {
    #[command(subcommand)]
    command: Command,
...
enum Command {
    ...
    AcpAgent {
        ...
        /// holds the reason, and `heddle chat` having nobody to ask for
...
#[derive(Args)]
pub struct SiloArgs {
    /// Directory holding the silos. Defaults to $HEDDLE_ROOT.
...
impl SiloArgs {
    /// `--root`, else `$HEDDLE_ROOT`, else a loud refusal. v0 has no config file
    ...
    pub fn root(&self) -> Result<PathBuf> {
        self.root.clone().map(Ok).unwrap_or_else(|| {
            std::env::var_os("HEDDLE_ROOT")
                .map(Into::into)
                .ok_or_else(|| {
                    HeddleError::Storage("no silo root: pass --root or set HEDDLE_ROOT".into())
                })
        })
    }
}

/// The knobs `heddle chat` needs beyond the silo and the model.
#[derive(Args)]
pub struct ChatArgs {
    #[command(flatten)]
    model: wiring::ModelArgs,
    ...
    /// mid-conversation" being an unanswered question for `heddle acp-agent` is
    /// most of it.
    #[command(flatten)]
...
enum LedgerCommand {
    ...
    Show {
        #[command(flatten)]
        silo: SiloArgs,
        /// The step id, as printed by `heddle ledger log`.
...
struct PruneSelector {
    /// One profile, named as `heddle sandbox list` prints it.
    #[arg(long, value_name = "NAME")]
    profile: Option<String>,
    /// Every profile heddle made on this machine.
    #[arg(long)]
    all: bool,
}
