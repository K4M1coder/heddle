//! `heddle secret set|delete` — the operator's way into the platform credential
...
use heddle_core::{HeddleError, Result, SecretRef};
...
fn read_value() -> Result<String> {
    if std::io::stdin().is_terminal() {
        return Err(HeddleError::Secret(
            "refusing to read a secret from a terminal: pipe it instead, e.g. \
             `printf %s \"$TOKEN\" | heddle secret set <REFERENCE>`"
                .into(),
        ));
    }
    ...
    std::io::stdin()
        ...
        .map_err(|e| HeddleError::Secret(format!("could not read the value from stdin: {e}")))?;
