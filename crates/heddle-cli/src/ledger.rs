//! `heddle ledger log|show|verify` — a rendering of `Ledger`'s read model.
//!
...
use crate::SiloArgs;
use heddle_core::{HeddleError, Ledger, Result, Step, StepKind};
...
/// from the hashed bytes, which is the rule `heddle-silo`'s store states for
/// itself.
fn kind_name(kind: &StepKind) -> Result<String> {
    match serde_json::to_value(kind)? {
        ...
        other => Err(HeddleError::Storage(format!(
...
/// wart, recorded in `specs/011-heddle-cli/spec.md`.
