...
//!
//! `heddle chat` and `heddle acp-agent` reach the same provider under the same
...
#[cfg(windows)]
use heddle_connectors::RunDirs;
use heddle_core::{
    HeddleError, LoopBudget, ProgressProbe, Redactor, Result, SecretProvider, SecretRef,
    SecretValue, ToolAccess, ToolCall, ToolOutcome, ToolPolicy, ToolSpec, ToolTransport,
};
use heddle_gateway::{LocalEndpoint, OpenAiCompatClient, ProviderTable, Router};
...
pub struct ModelArgs {
    ...
    pub model: String,
    /// OpenAI-compatible base URL. Defaults to $HEDDLE_MODEL_BASE_URL, else
...
impl ModelArgs {
    /// `--base-url`, else `$HEDDLE_MODEL_BASE_URL`, else Ollama's default, run
    ...
    pub fn endpoint(&self) -> Result<LocalEndpoint> {
        let base_url = match &self.base_url {
            Some(url) => url.clone(),
            None => {
                std::env::var("HEDDLE_MODEL_BASE_URL").unwrap_or_else(|_| DEFAULT_BASE_URL.into())
            }
        };
...
///
/// Flattened into `heddle chat` and nowhere else in this slice. `heddle
...
impl ToolArgs {
    ...
    ///
    /// `heddle chat` gets this for free from [`ToolArgs::transport`], which it
    /// calls at the same point in its sequence. `heddle acp-agent` cannot: it
    ...
    /// `heddle chat`'s allowlist: the read-only tools, and **not** `fs_write`.
    ///
    ...
    /// `heddle acp-agent`'s allowlist: everything [`ToolArgs::chat_policy`]
    ...
    /// protocol error, `RmcpToolTransport` maps to `HeddleError::Tool`, and
...
/// Flattened into `heddle acp-agent` and nowhere else — [`ToolArgs::chat_policy`]
...
impl RunArgs {
    ...
    /// owns the *operator's* answer. Below it, `heddle-sandbox` refuses on the
