//! An ACP facade over the native Heddle loop (ADR-0003 decision 2, ADR-0004 D3).
//!
//! This is the only crate in the product that names the Agent Client Protocol.
//! `heddle-core` reaches the outside world through the ports it defines and never
//! depends on this crate, exactly as `heddle-mcp` relates to MCP.
//!
...
/// silo-backed chain (`heddle_silo::Silo::ledger`) instead of an in-memory one.
...
///
/// A session owns N Heddle runs, one per `session/prompt`, with run ids
...
pub struct HeddleSession<C: ModelClient, P: ProgressProbe, T: ToolTransport> {
    id: SessionId,
...
type NativeLoop<C, P, T> =
    heddle_core::NativeLoop<CancellableModel<C>, P, AcpPermissionTransport<T>>;

impl<C: ModelClient, P: ProgressProbe, T: ToolTransport> HeddleSession<C, P, T> {
    fn new(id: SessionId, parts: SessionParts<C, P, T>, connection: ConnectionTo<Client>) -> Self {
        ...
        HeddleSession {
            id,
            engine: heddle_core::NativeLoop::new(client, parts.probe, gateway, parts.redactor),
            ledger: parts.ledger,
            budget: parts.budget,
            prompts: 0,
            cancelled,
...
struct Registered<C: ModelClient, P: ProgressProbe, T: ToolTransport> {
    session: Arc<Mutex<HeddleSession<C, P, T>>>,
...
/// Serves the ACP agent side: one `HeddleSession` per `session/new`, built from
/// `factory`. Cloneable, so a caller keeps a handle on the sessions it serves.
pub struct HeddleAgent<C: ModelClient, P: ProgressProbe, T: ToolTransport, F> {
    factory: Arc<Mutex<F>>,
    sessions: Sessions<C, P, T>,
    next_id: Arc<AtomicU64>,
}

impl<C: ModelClient, P: ProgressProbe, T: ToolTransport, F> Clone for HeddleAgent<C, P, T, F> {
    fn clone(&self) -> Self {
        HeddleAgent {
...
impl<C, P, T, F> HeddleAgent<C, P, T, F>
where
    ...
    F: FnMut() -> Result<SessionParts<C, P, T>> + Send + 'static,
{
    ...
    pub fn session(&self, id: &SessionId) -> Option<Arc<Mutex<HeddleSession<C, P, T>>>> {
        self.sessions
            .lock()
    ...
    fn open(&self, connection: ConnectionTo<Client>) -> Result<SessionId> {
        ...
        let session = HeddleSession::new(id.clone(), parts, connection);
    ...
    /// `heddle-mcp`'s `RmcpToolTransport` owns its runtime — so a CLI does not
    ...
    pub fn serve_stdio(self) -> Result<()> {
        futures::executor::block_on(self.serve(Stdio::new()))
            .map_err(|e| heddle_core::HeddleError::Protocol(format!("acp stdio: {e}")))
    }
...
/// ACP prompts are a block list; `heddle_core::Content` carries text only, so a
        ...
        match block {
            ...
            _ => {
                return Err(Error::invalid_params().data(serde_json::json!(
                    "heddle accepts text content blocks only in this version"
                )))
            }
        }
    }
