...
//! to finish; `heddle-sandbox`'s launcher, which polls it while a `proc_run`
...
//!
//! `heddle-sandbox`'s launcher is why the flag is supplied to a session rather
...
use heddle_core::{
    HeddleError, ModelClient, Result, TextSink, TurnRequest, TurnResponse, WireExchange,
};
...
impl<C: ModelClient> ModelClient for CancellableModel<C> {
    fn turn(&mut self, req: &TurnRequest) -> Result<TurnResponse> {
        if self.cancelled.load(Ordering::SeqCst) {
            return Err(HeddleError::Model("session cancelled by client".into()));
        }
        self.inner.turn(req)
    }
