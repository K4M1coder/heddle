...
use skein_core::{Result, SecretProvider, SecretRef, SkeinError};
...
impl ProviderTable {
    ...
    ///
    /// Every failure comes back as `SkeinError::Model` naming the path,
    ...
    pub fn from_path(path: &Path) -> Result<ProviderTable> {
        ...
        ProviderTable::from_toml_str(&text)
            .map_err(|e| SkeinError::Model(format!("{}: {e}", path.display())))
    }
    ...
    pub fn from_toml_str(text: &str) -> Result<ProviderTable> {
        let raw: RawTable = toml::from_str(text)
            .map_err(|e| SkeinError::Model(format!("the provider table is not valid: {e}")))?;
