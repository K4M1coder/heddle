...
use skein_core::{
    Message, ModelClient, Result, SecretProvider, SecretRef, SecretValue, SkeinError, TurnRequest,
};
use skein_gateway::{ProviderKind, ProviderRoute, ProviderTable, Router};
...
const TOKEN_REF: &str = "keychain://skein-test/cloud-primary";
...
///
/// Deliberately **not** `skein-silo`'s `TestSecret`: that fixture is backed by
/// the real OS keychain and lives in a crate `skein-gateway` must not depend on
...
impl SecretProvider for FakeSecrets {
    fn resolve(&self, secret: &SecretRef) -> Result<SecretValue> {
        ...
        Err(SkeinError::Secret(format!("no such secret: {}", secret.0)))
    }
    ...
    assert_eq!(
        ...
        &[
            ...
            ProviderRoute {
                ...
                credential: Some(SecretRef("keychain://skein/cloud-primary".into())),
            },
        ],
...
base_url = "https://api.example.invalid/v1"
model = "gpt-4o-mini"
credentials = "keychain://skein/cloud-primary"
"#,
    )
...
fn a_missing_providers_file_is_refused_by_name() {
    let missing = std::env::temp_dir().join("skein-no-such-providers-file.toml");
    let err = ProviderTable::from_path(&missing).expect_err("the file does not exist");

    let message = err.to_string();
    assert!(
        message.contains("skein-no-such-providers-file.toml"),
