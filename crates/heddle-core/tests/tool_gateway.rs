...
use heddle_core::{
    replay_tool_calls, HeddleError, Ledger, Redactor, Result, StepKind, ToolAccess, ToolCall,
...
impl ToolTransport for CountingTransport {
    fn call(&mut self, call: &ToolCall) -> Result<ToolOutcome> {
        ...
        if self.fail {
            return Err(HeddleError::Tool("downstream exploded".into()));
        }
        Ok(ToolOutcome {
...
fn an_unapproved_mutating_tool_is_still_advertised() {
    ...
    assert!(
        matches!(err, HeddleError::ToolDenied { ref reason, .. } if reason.contains("approval")),
...
fn a_catalogue_that_cannot_be_read_is_an_error_not_an_empty_advertisement() {
    ...
    assert!(matches!(err, HeddleError::Tool(_)), "{err:?}");
}
...
fn denied_mutating_tool_never_reaches_the_transport() {
    ...
    assert!(
        matches!(err, HeddleError::ToolDenied { ref tool, .. } if tool == "fs_write"),
...
/// A `CallToolResult` as `heddle-mcp` hands one over: the whole result,
/// serialized (`heddle-mcp/src/lib.rs`). `text` is the decoded string the tool
...
fn transport_error_leaves_the_chain_verifiable() {
    ...
    assert!(
        matches!(err, HeddleError::Tool(_)),
...
fn a_secret_in_a_tool_name_is_redacted_from_the_attempt_and_the_approval() {
    ...
    assert!(
        matches!(err, HeddleError::ToolDenied { ref tool, .. } if tool == &name),
...
fn unlisted_tool_is_denied_even_though_it_is_not_mutating() {
    ...
    assert!(
        matches!(err, HeddleError::ToolDenied { ref tool, .. } if tool == "shell_exec"),
