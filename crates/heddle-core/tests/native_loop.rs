...
use heddle_core::{
    Exit, HeddleError, Ledger, LoopBudget, LoopController, Message, ModelClient, NativeLoop,
    ProgressProbe, Redactor, Result, Role, StepKind, ToolAccess, ToolCall, ToolGateway,
...
impl ToolTransport for RecordingTransport {
    fn call(&mut self, call: &ToolCall) -> Result<ToolOutcome> {
        ...
        match &self.mode {
            ...
            TransportMode::Fail => Err(HeddleError::Tool("downstream exploded".into())),
    ...
    fn list(&mut self) -> Result<Vec<ToolSpec>> {
        self.lists += 1;
        if self.list_fails {
            return Err(HeddleError::Tool("the server would not enumerate".into()));
        }
...
fn provider_error_leaves_the_chain_verifiable() {
    ...
    assert!(matches!(err, HeddleError::Model(_)), "got {err:?}");
...
fn tool_transport_failure_propagates_and_leaves_the_chain_verifiable() {
    ...
    assert!(matches!(err, HeddleError::Tool(_)), "got {err:?}");
    assert!(
...
fn the_advertised_catalogue_reaches_every_turn_of_the_run() {
    ...
    // And the chain holds what the model was told, so `heddle ledger show` can
