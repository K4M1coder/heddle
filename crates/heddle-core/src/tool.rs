...
/// load-bearing rather than ergonomic: `heddle-acp` reconstructs a `ToolCall`
...
impl ToolCall {
    ...
    /// through [`ToolCall::with_id`], because `heddle-gateway` supplies an id
...
impl Redactor {
    ...
    /// whenever a provider supplies one: `heddle-gateway`'s OpenAI-compatible
...
impl<T: ToolTransport> ToolGateway<T> {
    ...
    /// Withholding it here would make `heddle-acp`'s permission decorator
    ...
    pub fn call_captured(
        ...
        if let Decision::Deny { .. } = decision {
            return Err(HeddleError::ToolDenied {
...
pub fn replay_tool_calls(ledger: &Ledger, run_id: &str) -> Result<Vec<CapturedResult>> {
    ledger
        ...
        .map(|s| serde_json::from_str(&s.payload).map_err(HeddleError::from))
        .collect()
}
