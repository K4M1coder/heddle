...
| "does ureq expose the exact bytes sent, or must skein-gateway serialize once for capture and again for the actual send… decide and justify" | **The dilemma does not arise.** `turn` already serializes exactly once into `let body: String`, passes `&body` to `post`, which calls `.send(body)`. ureq 3.4.0's `impl AsSendBody for &str` (`send_body.rs:300-306`) wraps the slice as `BodyInner::ByteSlice` with `Some(self.len())` as content-length — no copy, no transformation, no re-encode. Capturing `body` *is* capturing the transmitted buffer. See D2. |
...
`crates/skein-core/src/native_loop.rs`'s `NativeLoop::run` appends `StepKind::LlmRequest` from
`&req: &TurnRequest` and `StepKind::LlmResponse` from `&resp: &TurnResponse`. Both are **skein-core's
...
`crates/skein-gateway/src/lib.rs`'s `impl ModelClient for OpenAiCompatClient::turn`, and are dropped
...
between them. `crates/skein-gateway/tests/governed_run.rs` states this in a comment on a live
assertion:
...
`crates/skein-core/src/ledger.rs`'s `enum StepKind` gains a single variant `WireExchange`
(serde name `"wire_exchange"`, from the existing `#[serde(rename_all = "snake_case")]`).

Payload: a new `WireExchange` struct in `crates/skein-core/src/model.rs`, beside `TurnRequest` /
...
- `crates/skein-silo/src/ledger_store.rs`'s `SCHEMA` stores `kind TEXT NOT NULL` with **no CHECK
  constraint and no enum table**; `load` does `serde_json::from_str::<StepKind>(&kind)`. New names
  round-trip; old names are untouched.
- `crates/skein-cli/src/ledger.rs`'s `kind_name` derives the column from `serde_json::to_value(kind)`
  rather than matching the enum, so `skein ledger log|show` renders the new kind with **no CLI
  change at all**.
- `crates/skein-acp/src/lib.rs`'s `project_updates` matches four kinds and ends `_ => {}`, so the new
...
- The **response** capture is byte-identical to the string the parser consumed. That string is
  ...
  the bytes skein-core acted on — which is the auditable claim that matters. `gzip` is not a factor:
  `skein-gateway`'s `Cargo.toml` declares `ureq = { workspace = true }` against a workspace entry of
...
`crates/skein-core/src/model.rs`'s `trait ModelClient`:
...
- **Every existing `ModelClient` implementation compiles and behaves identically.** The default body
  returns `None`. `ScriptedModel` (`skein-core/tests/native_loop.rs`), and every stub across
  `skein-connectors`, `skein-mcp`, `skein-silo` and `skein-acp` tests, is a client with **no wire** —
...
Add to `crates/skein-core/src/tool.rs`'s `impl Redactor`:
...
**T1 — RED, the headline claim.** In `crates/skein-gateway/tests/governed_run.rs`, add a test beside
...
**T2 — RED, redaction, with a quote-bearing secret.** In `crates/skein-core/tests/native_loop.rs`,
beside `the_redacted_llm_payloads_are_still_parseable_turn_request_and_response`. This test needs a
real wire, so it belongs in `skein-gateway/tests/governed_run.rs` instead — build a `NativeLoop`
...
`crates/skein-silo/tests/silo_ledger.rs` (which already owns the persist/reopen tests). Build a
...
with a body `ChatResponse` cannot parse. `run` returns `Err(SkeinError::Model(_))`, and the chain
...
**T5 — GREEN, `skein-core`.** `ledger.rs`: the `WireExchange` variant. `model.rs`: the
...
**T6 — GREEN, `skein-gateway`.** `OpenAiCompatClient` gains `last_exchange: Option<WireExchange>`
...
- `crates/skein-gateway/tests/governed_run.rs` — the two-iteration `kinds` vector in
  `an_end_to_end_run_against_a_stub_provider_lands_on_the_chain` (`WireExchange` after each
  `LlmRequest`).
- `crates/skein-cli/tests/cli_chat.rs` — **5** kind vectors containing `"llm_request"`, and **4**
  `"… ok\t<n> steps"` counts (currently 5, 9, 12, 12).
- `crates/skein-cli/tests/cli_acp_agent.rs` — **10** kind vectors containing `"llm_request"`, and
...
`crates/skein-gateway/tests/openai_compat.rs`, following the house convention exactly
(`#[ignore = "needs a real local provider; set SKEIN_LIVE_MODEL to run"]`, model read from
`SKEIN_LIVE_MODEL`, doc comment showing the PowerShell invocation), asserting the captured
...
`skein chat --root … --silo … --base-url http://localhost:11434/v1 --model … --prompt …`, then
`skein ledger log` to find the `wire_exchange` step and `skein ledger show <id>` to read it, and
...
`BudgetSpent` payload, and (iii) `skein ledger verify` reports `ok`. Cross-checking (i) and (ii)
...
| Test | Home | Proves |
|---|---|---|
| exchange bytes equal the stub's observed bytes | `skein-gateway/tests/governed_run.rs` | acceptance 2 — capture is byte-identical to a real socket exchange, both directions |
| quote-bearing secret is scrubbed from the exchange | `skein-gateway/tests/governed_run.rs` | acceptance 3 + D4 — the escaped-needle hole is closed, not merely the literal one |
| plain secret is scrubbed from the exchange | `skein-gateway/tests/governed_run.rs` | acceptance 3 — parity with `LlmRequest`/`LlmResponse` |
| pre-023 chain persisted, reopened, ids unchanged, then extended | `skein-silo/tests/silo_ledger.rs` | acceptance 4 — upgrade shape, not fresh-chain |
| unreachable provider appends no exchange | `skein-gateway/tests/governed_run.rs` (existing test, extended) | the chain never claims bytes crossed when none did |
| non-2xx / unparseable body still records the exchange | `skein-gateway/tests/governed_run.rs` | the failure case that has no other witness |
| live provider exchange carries real `usage` | `skein-gateway/tests/openai_compat.rs`, `#[ignore]`d | acceptance 5 |
...
in `skein-core`; `lib.rs` in `skein-gateway`) and three test files updated for genuinely-moved
assertions. `skein-silo`, `skein-cli`, `skein-acp`, `skein-mcp`, `skein-connectors` and
`skein-sandbox` **sources** are untouched. No new dependency, no `Cargo.toml` change, no `#[cfg]`,
no new CLI surface.
...
- **`replay_tool_calls`-style replay from `WireExchange`**, a `skein ledger diff` comparing raw and
