# Feature Specification: `heddle sandbox list` / `heddle sandbox prune` (v0 slice)
...
removes either today, by design. A `heddle sandbox prune` subcommand — or an explicit decision that
...
AppContainer **profile** named `heddle-` + 16 hex characters of `sha256(canonical root path)`, and
...
One new subcommand group, `heddle sandbox`, with two members.

`heddle sandbox list` prints one tab-separated line per (profile, directory) — the profile name, its
...
`heddle sandbox prune --profile <NAME>` (or `--all`) revokes that profile's SID from each recorded
directory's DACL and then deletes the AppContainer profile. It reports what it did per directory,
one line each. It requires an explicit selector: a bare `heddle sandbox prune` is a usage error, not
...
`%LOCALAPPDATA%\Packages\<profile-name>\heddle-grants` — the folder Windows itself created for the
...
2. **The record lives beside the profile, and that location is measured, not chosen for taste.**
   `icacls` on this machine (Windows 11 Pro 10.0.26200): `…\Packages\heddle-<hash>` names
...
3. **`prune` proves ownership structurally, and deliberately does not trust the record.** Three
   layers: a **name gate** refusing anything outside `^heddle-[0-9a-f]{16}$` before any Win32 call;
...
5. **Every profile made before this slice has no record, and `prune` says so rather than refusing.**
   Measured on this machine: **1009** `heddle-<16 hex>` folders at this run's start, growing by
...
6. **`heddle sandbox` is present on every platform and refuses loudly off Windows.** This is the
...
- **FR-001** `Sandbox::create` writes `%LOCALAPPDATA%\Packages\<profile>\heddle-grants` — one
...
- **FR-004** `heddle_sandbox::grants()` returns one `Grant` per `%LOCALAPPDATA%\Packages` entry whose
  name matches `^heddle-[0-9a-f]{16}$` and nothing else. `Grant::dirs` is `None` for a profile with
...
- **FR-007** `heddle_sandbox::prune(profile)` refuses a name outside `^heddle-[0-9a-f]{16}$` with an
...
- **FR-013** `heddle sandbox --help` documents `list` and `prune` on **every** platform, and
  `heddle sandbox prune --help` documents `--profile` and `--all`.
- **FR-014** `--profile` and `--all` are mutually exclusive and one is required. A bare
  `heddle sandbox prune` is a clap usage error naming both.
- **FR-015** `heddle sandbox list` prints five tab-separated columns unconditionally —
...
- **FR-016** `heddle-cli` depends on `heddle-sandbox` **unconditionally**. The measured cost off
  Windows is zero new third-party packages: `heddle-sandbox`'s only non-Windows dependency is
...
- **SC-001** After `Sandbox::create` over a `TempDir` root and a `TempDir` run-dir,
  `%LOCALAPPDATA%\Packages\<profile>\heddle-grants` exists and holds both absolute paths, root
  first. (`heddle-sandbox/tests/record.rs`)
- **SC-002** A second `create` over the same root with a different run-dir leaves a record holding
  the root and **both** run-dirs. (`heddle-sandbox/tests/record.rs`)
- **SC-003** `grants()` lists the created profile with both directories at `GrantState::Granted`,
  ...
  (`heddle-sandbox/tests/record.rs`)
- **SC-004** A profile whose record file has been removed is still listed, with `dirs: None`.
  (`heddle-sandbox/tests/record.rs`)
- **SC-005** Every profile `grants()` returns matches `^heddle-[0-9a-f]{16}$`, on a machine whose
...
- **SC-006** **Acceptance.** Create → both DACLs name the SID (read back through
  ...
  package folder is gone, and `grants()` no longer lists it. (`heddle-sandbox/tests/prune.rs`)
- **SC-007** **Acceptance.** `prune` on a directory it *does* rewrite leaves the root's allow-ACE
  ...
  (`heddle-sandbox/tests/prune.rs`)
- **SC-008** `prune("Microsoft.WindowsCalculator_8wekyb3d8bbwe")` and `prune("heddle-notahexstring")`
  are both `Err` naming the required shape, and the calculator's package folder still exists
  afterwards. (`heddle-sandbox/tests/prune.rs`)
- **SC-009** Pruning a recordless profile reports `unrecorded`, removes the profile folder, and
  leaves the root's ACE for that SID in place. (`heddle-sandbox/tests/prune.rs`)
- **SC-010** Off Windows, `grants()` and `prune("heddle-0000000000000000")` are both `Err`.
  (`heddle-sandbox/tests/absent.rs`, two of three CI legs)
- **SC-011** `heddle sandbox --help` names `list` and `prune`, and `heddle sandbox prune --help` names
  `--profile` and `--all`, on every platform. `heddle sandbox prune` with no selector, and with
  both, each exit nonzero. (`heddle-cli/tests/cli_sandbox.rs`)
- **SC-012** On Windows, a real profile created in-process appears on one `heddle sandbox list` line
  with its root path and `granted`; `heddle sandbox prune --profile <name>` exits 0; a second `list`
  no longer names it. (`heddle-cli/tests/cli_sandbox.rs`)
- **SC-013** Off Windows, `heddle sandbox list` exits nonzero with a message naming the platform.
  (`heddle-cli/tests/cli_sandbox.rs`)
- **SC-014** The `%LOCALAPPDATA%\Packages\heddle-*` count is **equal** before and after a full
...
- **Cleanup of anything `heddle-sandbox` does not create** — no silo pruning, no ledger compaction,
