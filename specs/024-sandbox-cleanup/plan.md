# Implementation Plan: `heddle sandbox list` / `heddle sandbox prune` (v0 slice)
...
- The `%LOCALAPPDATA%\Packages\heddle-*` DACL readings in *The AppContainer folder layout, measured*
...
`Sandbox::create` (`crates/heddle-sandbox/src/profile.rs`) does two things that outlive the process:

1. It creates a Windows **AppContainer profile** named `heddle-` + the first 16 hex characters of
...
plainly (*"Nothing removes either today, by design. A `heddle sandbox prune` subcommand — or an
...
```
$ ls -d "$LOCALAPPDATA/Packages"/heddle-* | wc -l
955
```

955 orphaned `heddle-<16 hex>` profile folders, each containing exactly one entry, `AC`. Most were
produced by the test suite itself — `crates/heddle-sandbox/tests/{profile,launch,escape}.rs` and
`crates/heddle-connectors/tests/connector.rs` each call `Sandbox::create` over a fresh `TempDir`, so
...
Read in full: `crates/heddle-sandbox/src/{profile.rs,lib.rs}`, `crates/heddle-sandbox/tests/profile.rs`,
`crates/heddle-cli/src/{main.rs,acp.rs,wiring.rs,secret.rs}`, `crates/heddle-cli/Cargo.toml`,
`crates/heddle-sandbox/Cargo.toml`, workspace `Cargo.toml`; targeted reads of
`crates/heddle-cli/src/ledger.rs`, `crates/heddle-cli/tests/cli_secret.rs`,
`crates/heddle-sandbox/src/launch.rs`, `crates/heddle-connectors/src/server.rs`; specs 019 and 020
...
- `ConvertStringSidToSidW` is already imported by `crates/heddle-sandbox/src/launch.rs`, and that
...
| Object | Trustees |
|---|---|
| `…\Packages\heddle-000986f7bfe73a3d` | `NT AUTHORITY\SYSTEM (I)(OI)(CI)(F)`, `BUILTIN\Administrators (I)(OI)(CI)(F)`, `<user> (I)(OI)(CI)(F)` — **no AppContainer SID** |
| `…\Packages\heddle-000986f7bfe73a3d\AC` | the unresolvable AppContainer SID at `(OI)(CI)(CR)(F)`, plus the three above, plus `Mandatory Label\Low Mandatory Level:(NW)` |
...
`crates/heddle-sandbox/src/launch.rs` already establishes `LOCALAPPDATA` as load-bearing and measured
...
- The request suggests *"a small manifest file under the silo root or a well-known Heddle state
...
`crates/heddle-sandbox/**`, `crates/heddle-cli/src/main.rs` and `crates/heddle-cli/Cargo.toml` are
...
`heddle-gateway`/`heddle-core`/ledger paths this slice never enters. File overlap: **none**, except
`crates/heddle-cli/src/main.rs`'s `enum Command` if 023 also adds a subcommand — a one-variant
...
`heddle sandbox list` and `heddle sandbox prune`, backed by two new public functions in
`heddle-sandbox` and a per-profile record file that `Sandbox::create` writes beside the profile
folder Windows itself created.

### D1 — The record is a plain text file, `heddle-grants`, in the profile's own package folder

`Sandbox::create` writes `%LOCALAPPDATA%\Packages\<profile-name>\heddle-grants`: one absolute path
...
2. **Not invented.** `SiloArgs::root` records this project's standing refusal to guess a data
   ...
   Heddle chose — it is where Windows put the profile Heddle created. Recording beside it follows the
   OS rather than inventing a `%LOCALAPPDATA%\heddle\` the operator never named.
...
`serde_json` is a workspace dependency but is not in `heddle-sandbox`'s graph, and this does not earn
it.
...
It would also force a Heddle-crate dependency into `heddle-sandbox`, which slices 019 and 020 both
certify as *"a leaf depending on no Heddle crate"* (Constitution IV).

**Rejected: a `%LOCALAPPDATA%\heddle\` state directory.** Machine-scoped and out of the child's
...
**Rejected: no record at all; the operator re-states the paths** (`heddle sandbox prune --fs-root X
...
### D2 — Enumeration is a directory scan of `%LOCALAPPDATA%\Packages` for `heddle-` + 16 hex
...
`^heddle-[0-9a-f]{16}$`, and for each reads `heddle-grants` if present.
...
1. **Name gate.** `prune(profile)` refuses anything not matching `^heddle-[0-9a-f]{16}$` before it
...
worst make `prune` revoke a `heddle-<hash>` SID's ACE from some directory — an ACE only Heddle could
...
`heddle-` + 16 lowercase hex is a namespace nothing else on a Windows machine produces. For revoking
an *ACE*: the name is not even the relevant question, because the trustee **is** the profile Heddle
...
### D5 — `heddle sandbox {list,prune}`: present on every platform, refusing loudly off Windows
...
(`heddle sandbox prune`), so no operator-facing term is invented.

*Presence.* Slice 019 already decided the analogous case explicitly — `specs/019-…/tasks.md` T9:
*"`--allow-run` appears in `heddle acp-agent --help` on **every** platform, deliberately: the flag is
...
a typo. So `heddle sandbox --help` works everywhere, and `list`/`prune` return a `NO_BACKEND`-shaped
`Err` off Windows.
...
bare `heddle sandbox prune` is a usage error, never a machine-wide delete.

### D6 — Public API added to `heddle-sandbox`
...
/// One AppContainer profile Heddle created. `dirs` is `None` when the profile
...
`heddle-cli` gains `heddle-sandbox = { path = "../heddle-sandbox" }` as an **unconditional**
dependency. Measured cost off Windows is zero new packages: `heddle-sandbox`'s only non-Windows
dependency is `sha2`, already in the graph — slice 019's dependency-drift table measured exactly this
(`+1` on Linux, and that one is `heddle-sandbox` itself). Routing through `heddle-connectors` instead
...
subcommand is a call onto `heddle-core`/`heddle-silo` plus a rendering of the result"*.

### D7 — Output format

`heddle sandbox list`: one **five-column tab-separated** line per (profile, directory), columns fixed
...
`heddle sandbox prune`: one line per action, in `secret::delete`'s shape — `revoked <path>`,
...
- **T1 — control baseline.** Re-measure, do not quote: `cargo fmt --all --check`,
  ...
  `ls -d "$LOCALAPPDATA/Packages"/heddle-* | wc -l` immediately before and after that test run. The
...
- **T2 — types and signatures, no behaviour.** Add `GrantKind`, `GrantState`, `GrantedDir`, `Grant`,
  `Pruned`, `grants()` and `prune()` to `crates/heddle-sandbox/src/lib.rs` with `todo!("T3")` /
  `todo!("T5")` Windows bodies and real `#[cfg(not(windows))]` refusal arms; add the new module
  `crates/heddle-sandbox/src/record.rs`; add `heddle-sandbox` to `crates/heddle-cli/Cargo.toml`. This
...
- **T3 — RED→GREEN: the record.** New `crates/heddle-sandbox/tests/record.rs` (`#![cfg(windows)]`,
  ...
  - `a_created_sandbox_records_its_root_and_run_dirs_beside_its_profile` — `Sandbox::create` over a
    `TempDir` root plus a `TempDir` run-dir; read `%LOCALAPPDATA%\Packages\<profile>\heddle-grants`
...
- **T4 — RED→GREEN: `grants()`.** Same file.
  ...
  - `a_profile_with_no_record_is_listed_with_no_directories` — create, then delete the
    `heddle-grants` file by hand (the shape of all 955 legacy profiles); assert `grants()` still lists
    it with `dirs: None`.
  - `nothing_but_a_heddle_hash_name_is_listed` — every returned `profile` matches
    `^heddle-[0-9a-f]{16}$`. A real machine-wide assertion, since `%LOCALAPPDATA%\Packages` holds
...
- **T5 — RED→GREEN: `prune()` removes, and removes only its own.** First extract `tests/profile.rs`'s
  ...
  `crates/heddle-sandbox/tests/dacl/mod.rs`, and `mod dacl;` from both files — one copy of a subtle
  `GetAce`/`MapGenericMask` helper rather than two. `profile.rs`'s assertions must stay
  byte-identical; the measured control is `cargo test -p heddle-sandbox --test profile` unchanged
  before and after the extraction. Then, in new `crates/heddle-sandbox/tests/prune.rs`
  (`#![cfg(windows)]`):
  ...
  - `pruning_leaves_every_ace_it_did_not_write` — the refusal test. Snapshot `allow_aces(root)`
    **before** `Sandbox::create`; additionally place an unrelated ACE on the run-dir for
    `ALL APPLICATION PACKAGES` (`S-1-15-2-1`, well-known and emphatically not a `heddle-` profile);
  ...
  - `prune_refuses_a_name_it_could_not_have_created` —
    `prune("Microsoft.WindowsCalculator_8wekyb3d8bbwe")` and `prune("heddle-notahexstring")` are both
...
- **T6 — RED→GREEN: the non-Windows refusal.** `crates/heddle-sandbox/tests/absent.rs`
  (`#![cfg(not(windows))]`): `grants()` and `prune("heddle-0000000000000000")` are both `Err`. Runs on
  ...
  `cargo check -p heddle-sandbox --target x86_64-unknown-linux-gnu --all-targets`, which slice 019
  confirmed works for this crate precisely because its dependency graph is Heddle-free.
...
- **T7 — RED→GREEN: the CLI.** `Command::Sandbox { command: SandboxCommand }` in `main.rs` with
  ...
  `crates/heddle-cli/src/sandbox.rs` doing the D7 rendering and nothing else. New
  `crates/heddle-cli/tests/cli_sandbox.rs`:
  - `sandbox_list_and_prune_are_documented_on_every_platform` — `heddle sandbox --help` names both
    subcommands; `heddle sandbox prune --help` names `--profile` and `--all`. No `#[cfg]`: this is
  ...
  - `#[cfg(windows)] a_real_grant_is_listed_and_pruned_through_the_binary` — create a sandbox
    in-process via `heddle_sandbox::Sandbox::create` over a `TempDir`; run the real `heddle sandbox
    list` subprocess and assert the profile name and the root path appear on one tab-separated line
    with `granted`; run `heddle sandbox prune --profile <name>`, assert exit 0, and assert a second
    `list` no longer names it. Deliberately **no Win32 in `heddle-cli`** — the DACL read-back is T5's
...
- **T8 — the test suite stops leaking profiles.** Add the same prune-on-`Drop` guard wherever
  `Sandbox::create` is called over a `TempDir`: `crates/heddle-sandbox/tests/{profile,launch,escape}.rs`
  and `crates/heddle-connectors/tests/connector.rs`. The measured acceptance is T1's counter — the
  `%LOCALAPPDATA%\Packages\heddle-*` count before and after a full `cargo test --workspace` must be
...
- **T9 — gates, dependency drift, control diff, close-out.** `cargo fmt --all --check`;
  ...
  Control diff: `git diff dev --stat -- crates/heddle-core/ crates/heddle-gateway/ crates/heddle-silo/
  crates/heddle-acp/ crates/heddle-mcp/ spikes/ rust-toolchain.toml` must be **empty**, and
  `crates/heddle-connectors/` must show T8's test-only change with nothing in its `src/`.
...
- **T10 — hand-verification against a real session.** Not part of the implementation run; performed
  ...
  slice 020's T10:
  1. Run a real `heddle acp-agent --silo … --model … --fs-root <dir> --allow-run --run-dir <toolchain>`
  ...
  3. `heddle sandbox list` — confirm the profile, both directories, and `granted` on both.
  4. `heddle sandbox prune --profile heddle-<hash>`.
  5. `icacls` both directories again — confirm the SID is gone and every other trustee is unchanged;
     confirm `%LOCALAPPDATA%\Packages\heddle-<hash>` no longer exists.
  6. Optionally `heddle sandbox prune --all` against the 955 legacy profiles, recording the count
...
`cargo check -p heddle-sandbox --target x86_64-unknown-linux-gnu --all-targets` for the non-Windows
arm. The `%LOCALAPPDATA%\Packages\heddle-*` count before/after a suite run is a **new** gate this
...
| `nothing_but_a_heddle_hash_name_is_listed` | `list` does not report the machine's Store packages |
...
| `prune_refuses_a_name_it_could_not_have_created` | `DeleteAppContainerProfile` is unreachable with a non-Heddle name |
...
**Blast radius.** `crates/heddle-sandbox/src/{lib.rs,profile.rs}` plus new `record.rs`;
`crates/heddle-cli/src/main.rs` plus new `sandbox.rs` and `Cargo.toml`; new tests in both crates; and
test-only edits in `crates/heddle-connectors/tests/connector.rs` (T8 only).
**`heddle-core`, `heddle-gateway`, `heddle-silo`, `heddle-acp`, `heddle-mcp`, `spikes/`, and every `src/`
file in `heddle-connectors` are untouched.** The governed `ToolGateway`/`Ledger`/ACP chain gains and
loses nothing: no `StepKind`, no protocol change, no network.

| Risk | Why it is bounded | If it bites |
|---|---|---|
| `prune` removes an ACE it did not create | Structurally impossible: `REVOKE_ACCESS` names one `TRUSTEE_IS_SID` — the SID derived from a `heddle-<16 hex>` name — so no other trustee's ACE is representable in the write. Proven by `pruning_leaves_every_ace_it_did_not_write`. | — |
| `prune` deletes a non-Heddle AppContainer profile | Name gate refuses anything outside `^heddle-[0-9a-f]{16}$` before any Win32 call. Proven by `prune_refuses_a_name_it_could_not_have_created`. | — |
...
- **Cleanup of anything `heddle-sandbox` does not create** — no silo pruning, no ledger compaction, no
...
- **Reopening whether `--run-dir` should grant an ACE at all** — `specs/020-…/tasks.md` leaves that
  ...
  should be made.
- **Touching `heddle-core`, `heddle-gateway`, `heddle-acp`, `heddle-mcp`, `heddle-silo`,
  `heddle-connectors`' `src/`, or `spikes/`** (ADR-0004 D2).
