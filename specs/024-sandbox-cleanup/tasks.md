# Tasks: `heddle sandbox list` / `heddle sandbox prune` (v0 slice)
...
- I Headless core ✅ the capability is two public free functions on `heddle-sandbox`
  (`grants`, `prune`); `heddle sandbox list|prune` is a call onto them plus a rendering, holding no
...
- IV Inverted coupling ✅ `heddle-sandbox` remains a leaf depending on no Heddle crate. `heddle-cli`
  gains an edge to it directly rather than through `heddle-connectors`, because `prune` is not an
...
- VI Security ✅ this is the slice that makes a destructive-by-nature capability safe by
  ...
  **not** one of them, so even a tampered record could at worst revoke a `heddle-<hash>` SID's own
...
- VII Neutrality ✅ one subcommand group, two functions, one plain-text record file. No new crate,
  no new `windows` feature, no serde. A silo-scoped manifest, a `%LOCALAPPDATA%\heddle\` state
...
- [x] **T1** control baseline: `cargo fmt --all --check`, `cargo clippy --workspace --all-targets --
      -D warnings`, `cargo test --workspace`, plus the `%LOCALAPPDATA%\Packages\heddle-*` count
...
- [x] **T2** types and signatures, no behaviour: `GrantKind`, `GrantState`, `GrantedDir`, `Grant`,
      `Pruned`, `grants()`, `prune()`, the new `record` module, and `heddle-sandbox` on
      `heddle-cli`'s dependency list
...
- `cargo test --workspace` — **250 passed, 0 failed, 6 ignored**: `acp_session` 16,
  ...
  `heddle-sandbox` `src/lib.rs` unit target 4 (`argv`), `escape` 4, `launch` 4, `profile` 3,
...
`heddle-sandbox/tests/{profile,launch,escape}.rs` and
`heddle-connectors/tests/{connector,run_server}.rs`. This is T8's control: it must become 0.
...
| Object | Trustees |
|---|---|
| `…\Packages\heddle-000986f7bfe73a3d` | `AUTORITE NT\Système (I)(OI)(CI)(F)`, `BUILTIN\Administrateurs (I)(OI)(CI)(F)`, `WINE\cthedrez (I)(OI)(CI)(F)` — **no AppContainer SID** |
| `…\Packages\heddle-000986f7bfe73a3d\AC` | the unresolvable AppContainer SID at `(OI)(CI)(CR)(F)`, plus those three, plus `Niveau obligatoire faible:(OI)(CI)(NW)` |
...
D:\Users\cthedrez\AppData\Local\Packages\heddle-d6db9923d5c88a60\heddle-grants:
...
---- nothing_but_a_heddle_hash_name_is_listed stdout ----
...
**T5** — `cargo test -p heddle-sandbox --test prune`, all four new tests, against `prune`'s
`todo!("T5")` body:

```
thread 'prune_refuses_a_name_it_could_not_have_created' panicked at
crates\heddle-sandbox\src\cleanup.rs:107:5:
...
- `cargo check -p heddle-sandbox --target x86_64-unknown-linux-gnu --all-targets` — clean, so the
  file compiles where it will run.
- `cargo clippy -p heddle-sandbox --target x86_64-unknown-linux-gnu --all-targets -- -D warnings` —
  clean.
...
`cargo check -p heddle-cli --target x86_64-unknown-linux-gnu` is **not** available on this machine —
that crate's graph needs a Linux C toolchain (`x86_64-linux-gnu-gcc`), which is what slice 019's
note implies by attributing the cross-check's availability to `heddle-sandbox`'s Heddle-free and
C-free graph. So `cli_sandbox.rs`'s `#[cfg(not(windows))]` arm was type-checked by temporarily
swapping the two `cfg`s in that file and running `cargo check -p heddle-cli --all-targets`, which
passed; the swap was reverted and is in no commit.

**T7** — `cargo test -p heddle-cli --test cli_sandbox`, the three new tests, before `main.rs` knew
the subcommand:

```
thread 'prune_without_a_selector_is_a_usage_error' panicked at
crates\heddle-cli\tests\cli_sandbox.rs:74:9:
...
after guarding only the files the plan lists, `heddle-sandbox` and `heddle-connectors` were each at
+0 and `heddle-cli` was still at **+2**.
...
2. **`dev` moved during this run.** It was `12c14f5` when this branch was reset onto it and is
   ...
   The predicted conflict does **not** exist: `git diff 12c14f5 dev -- crates/heddle-cli/` touches
...
3. **`granted_masks` moved back out of `tests/dacl/mod.rs`** into `tests/profile.rs`. The plan's T5
   ...
   and `cargo test -p heddle-sandbox --test profile` is 3 passed before and after.
...
4. **`record::read` collapses an empty record to `None`.** Not in the plan. A file that parses to
   ...
   a profile in `heddle sandbox list` with no line of its own — invisible in the listing an operator
   uses to decide what to prune.

5. **T8 covers three call sites the plan does not name.** The plan lists
   `heddle-sandbox/tests/{profile,launch,escape}.rs` and `heddle-connectors/tests/connector.rs`;
   `heddle-sandbox/tests/record.rs`, `heddle-connectors/tests/run_server.rs` and
   `heddle-cli/tests/cli_acp_agent.rs` also mint profiles, and without them the measured leak stops
   ...
   Two guard shapes, not one: `heddle-sandbox`'s tests hold the `Sandbox` and key on
...
6. **The risk the plan flagged did not bite.** `DeleteAppContainerProfile` removes the package
   folder even with the `heddle-grants` file inside it, measured by
...
- `cargo check -p heddle-sandbox --target x86_64-unknown-linux-gnu --all-targets` — clean.
- `%LOCALAPPDATA%\Packages\heddle-*` before and after that suite run — **1468 and 1468**, against
...
dependency edge this slice adds — `heddle-cli` to `heddle-sandbox` — is a path dependency already in
...
`git diff 12c14f5 --stat -- crates/heddle-core/ crates/heddle-gateway/ crates/heddle-silo/
crates/heddle-acp/ crates/heddle-mcp/ spikes/ rust-toolchain.toml Cargo.lock Cargo.toml` is
**empty**. `crates/heddle-connectors/` shows `tests/connector.rs` (+5), `tests/run_server.rs` (+7)
...
`crates/heddle-cli/tests/cli_sandbox.rs`'s own `windows::a_real_grant_is_listed_and_pruned_through_the_binary`
was re-run standalone and passed (`cargo test -p heddle-cli --test cli_sandbox`: 3 passed, 0 failed) —
a real `Sandbox::create`, then the real `heddle sandbox list`/`prune` subprocess, then a real DACL
read-back.

Independently of that test, a throwaway `heddle-sandbox` example called `Sandbox::create` directly
over a real `--fs-root` and a real `--run-dir` (`D:\…\heddle-t10-manual\{root,toolbin}`), removed
...
```
> heddle sandbox list | grep 4293476591
heddle-b04ff468c9ade33b  S-1-15-2-4293…  root     granted  …\heddle-t10-manual\root
heddle-b04ff468c9ade33b  S-1-15-2-4293…  run-dir  granted  …\heddle-t10-manual\toolbin

> heddle sandbox prune --profile heddle-b04ff468c9ade33b
revoked …\heddle-t10-manual\root
revoked …\heddle-t10-manual\toolbin
deleted profile heddle-b04ff468c9ade33b
```

After pruning: `heddle sandbox list` no longer names the profile; `icacls` on both directories shows
...
`%LOCALAPPDATA%\Packages\heddle-b04ff468c9ade33b` no longer exists. Every trustee present before
