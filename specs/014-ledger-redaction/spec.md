...
`LlmRequest`/`LlmResponse` path**, so `heddle ledger show` cannot print a conversation secret"*,
...
Since slice 012, `heddle chat` drives a real conversation through a real provider; since slice 013,
`heddle acp-agent` does the same from an editor. Both write every turn into a durable SQLite-backed
silo (slice 009), readable forever after with `heddle ledger show`. `ToolGateway::call_captured`
...
```
heddle chat      --silo <ID> [--root <PATH>] --model <NAME> [--base-url <URL>] [--redact <REFERENCE>]…
heddle acp-agent --silo <ID> [--root <PATH>] --model <NAME> [--base-url <URL>] [--redact <REFERENCE>]…
```

`--redact` is repeatable and takes a **reference** — `keychain://<service>/<account>`, the same shape
`heddle secret set` provisions — never a value. There is no `--redact-value`, for exactly the reason
`heddle secret set` has no `--value`: a secret in a flag lands in shell history and in process
listings.
...
1. **`heddle chat`'s stdout is unaffected.** It prints `run.final_message`, which is the raw
...
3. **`LlmRequest`/`LlmResponse` payload key order depends on the build graph, and every consumer
   ...
   any binary whose graph includes `heddle-acp` — `heddle-cli`, since it depends on `heddle-acp` —
...
4. **This is not a defence against a secret the operator never configured.** `Redactor` is an
   ...
   `heddle secret set` and named with `--redact` still lands in cleartext. This slice makes redaction
...
- **FR-005** `ToolGateway::call_captured` redacts the tool name in the `ToolCall` attempt, the
  ...
  receive the raw name, and `HeddleError::ToolDenied` still names it raw to the caller.
- **FR-006** `HeddleSession::new` clones the injected redactor into the gateway and passes the
  original to the loop. `heddle-acp`'s public API is otherwise unchanged.
- **FR-007** `heddle chat` and `heddle acp-agent` accept a repeatable `--redact <REFERENCE>`, resolved
  through `heddle-silo`'s `OsKeychain` as a `SecretProvider`, after the endpoint guard and before the
  silo is opened.
...
- **SC-006** `heddle chat --redact <ref>` prints the real answer on stdout and writes `***` to the
...
- **SC-008** `git diff dev -- crates/heddle-silo/ spikes/ .github/ rust-toolchain.toml` is empty
  except `crates/heddle-silo/tests/silo_ledger.rs`'s one mechanical `NativeLoop::new` argument.
  `heddle-silo`'s and `heddle-gateway`'s **product** code is untouched.
...
- **`heddle secret set` is the provisioning path.** `--redact` resolves references; it does not create
  them.

## Out of scope

- **Raw wire-byte capture** — the HTTP bodies `heddle-gateway` exchanges. A separate, already-named
  next-slice item.
...
- **A config file for secret references**, or a `$HEDDLE_…` environment fallback for `--redact`.
