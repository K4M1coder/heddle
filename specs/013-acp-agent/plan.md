# Implementation Plan: `skein acp-agent` — the ACP facade as a running process (v0 slice)
...
shared wiring module in the CLI, and one additive change each to `skein-acp` and `skein-core`.

`SkeinAgent::new` already requires `C: ModelClient + Send + 'static` and `OpenAiCompatClient`
...
1. **No tokio runtime is required, and none is added.** `crates/skein-acp/Cargo.toml` lists `tokio`
...
### D1 — `skein acp-agent` lives in `skein-cli`; the stdio transport stays inside `skein-acp`

`crates/skein-acp/src/lib.rs`'s module doc claims: *"This is the only crate in the product that
names the Agent Client Protocol."* Naming `agent_client_protocol::Stdio` in `skein-cli` would break
that claim (and Principle IV's boundary discipline) for one type. So `skein-acp` gains **one**
public method on the existing `impl<C, P, T, F> SkeinAgent<C, P, T, F>` block, beside `serve`:

```rust
pub fn serve_stdio(self) -> skein_core::Result<()>
```

implemented as `futures::executor::block_on(self.serve(Stdio::new())).map_err(…)`.

The precedent is exact: `crates/skein-mcp/src/lib.rs`'s `RmcpToolTransport` owns its runtime and
...
*Alternative rejected:* put `Stdio::new()` and `block_on` in `skein-cli/src/acp.rs`. It needs no
change to `skein-acp`, but it makes the CLI name ACP types and forces `agent_client_protocol::Error`
...
`skein-acp` gains `futures = { version = "0.3", default-features = false, features = ["std",
...
*blocking* work is on a plain `std::thread` that `skein-acp` already spawns. Zero new packages.

*Alternative rejected:* `tokio::runtime::Builder::new_current_thread()` — mirrors `skein-mcp`
...
### D3 — The factory becomes fallible: `F: FnMut() -> skein_core::Result<SessionParts<C, P, T>>`

This is the one public-API change to `skein-acp`, and it needs justifying (Principle IV).

Today `SkeinAgent::new(factory)` takes `F: FnMut() -> SessionParts<C, P, T>`. That was free while
...
`SkeinAgent::new`'s `F` bound; `SkeinAgent::open` returns `skein_core::Result<SessionId>`; the
`NewSessionRequest` handler maps the error with `respond_with_internal_error`, which the
`PromptRequest` handler already uses with a `SkeinError`; and the construction sites in
`crates/skein-acp/tests/acp_session.rs` gain `Ok(…)`. **No other crate constructs a `SkeinAgent` or
a `SessionParts`.**
...
### D4 — A shared `wiring` module in `skein-cli`, so the loopback guard has exactly one copy

`skein acp-agent` needs the same six knobs `skein chat` has (`--model`, `--base-url`, `--max-iters`,
...
that lived in `crates/skein-cli/src/chat.rs`.

New module `crates/skein-cli/src/wiring.rs` holds `ModelArgs` (a clap `#[derive(Args)]` carrying
...
VIII(b) and deny-by-default arguments. `NoTools`'s message is generalised from "`skein chat`" to
"this command", since it is now shared by two.

`ChatArgs` keeps `--prompt` and `--run-id` and gains `#[command(flatten)] model: ModelArgs`
**declared first**, so `skein chat --help` keeps its current flag order.
...
`session_id = skein-{n}` minted from `SkeinAgent`'s own `AtomicU64` starting at 1, so the first
session's first prompt is run `skein-1#1` — deterministic, and directly assertable in the test.
...
### D6 — `SkeinError::Protocol(String)`: one additive variant in `skein-core`
...
Nothing else in `skein-core` changes.

*Alternative rejected:* reuse `SkeinError::Tool`. It is what `AcpPermissionTransport` uses today —
...
`skein acp-agent` never writes to stdout: a single stray byte corrupts the JSON-RPC stream. It
writes exactly one stderr line at startup (naming the silo and the endpoint — the same courtesy
`skein chat`'s `run {run_id}` line provides) and nothing per turn. An ACP client is not obliged to
...
|---|---|
| `SkeinAgent::serve_stdio` (D1) | The alternative is naming `agent_client_protocol::Stdio` and `Error` in `skein-cli`, breaking the crate's own "only crate that names ACP" claim. |
...
| `crates/skein-cli/src/wiring.rs` (D4) | Two commands need the same NON-NEGOTIABLE Principle II guard. One implementation, with the five existing `cli_chat.rs` tests as the control. |
| `SkeinError::Protocol` (D6) | `serve_stdio`'s failure is neither a model failure nor a tool failure, and printing it as one would mislead an operator. |
