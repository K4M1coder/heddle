# Feature Specification: `skein acp-agent` — the ACP facade as a running process (v0 slice)

**Feature Branch:** `013-acp-agent` · **Created:** 2026-09-03 · **Status:** Implemented (v0 slice)
**Input:** `specs/012-model-gateway/tasks.md` "Next slice" — *"**`skein acp-agent`** — the stdio ACP
...
Slice 008 built a complete, tested ACP facade in `crates/skein-acp` — `SkeinAgent`, `SkeinSession`,
...
first real `ModelClient` and wired it into `skein chat`. Both halves are real; **nothing in the
workspace served ACP as a running process with a real model behind it.**

This slice ships the process. `skein acp-agent` serves the slice-008 facade on the executable's own
stdin/stdout, with `skein-gateway`'s loopback-only client in front of the model and `skein-silo`'s
...
```
skein acp-agent --silo <ID> [--root <PATH>] --model <NAME> [--base-url <URL>]
...
configured to launch that command gets a Skein agent whose every session is a governed `NativeLoop`
run against a loopback-only local model, recorded on the silo's hash-chained ledger and verifiable
afterwards with `skein ledger log|show|verify`.

**It does not add a tokio runtime, and does not need one.** `crates/skein-acp` lists `tokio` under
...
1. **The test suite needs no editor and no Ollama.** The end-to-end test is the real `skein` binary
...
2. **stdout is the protocol.** A single stray byte on stdout corrupts the JSON-RPC stream, so
   `skein acp-agent` writes nothing there but ACP frames. Its stderr is one startup line naming the
...
3. **`AcpPermissionTransport` is not reachable through this command, and the reason is the policy.**
   `skein_core::TurnRequest` is `{ run_id, messages }`, so no `tools` field goes on the wire, and
   `skein-cli` does not depend on `skein-mcp`, so there is no tool server to reach. The session is
...
4. **A session's read model is a snapshot taken at its own `session/new`.** `Ledger::open` calls
   ...
   reads the run just appended — and is the same property `skein chat` has.

## User Scenarios & Testing

### User Story 1 — An editor drives Skein, and the conversation is on the chain (P1)
As an operator, I point an ACP-speaking editor at `skein acp-agent` and get a governed agent whose
...
1. **Given** a local OpenAI-compatible provider on loopback and a temp silo root, **When** a real
   ACP client spawns `skein acp-agent --root <root> --silo alpha --model <name> --base-url <url>`
   as a subprocess and sends `initialize`, `session/new`, then two `session/prompt`s, **Then** the
   `InitializeResponse` comes back, `session/new` yields `SessionId("skein-1")`, each
   `PromptResponse.stop_reason` is `EndTurn`, and the `AgentMessageChunk` notifications carry the
   provider's two answers.
2. **Given** that connection has closed, **When** `skein ledger log --root <root> --silo alpha` is
   invoked as a **second process**, **Then** runs `skein-1#1` and `skein-1#2` are listed, each with
   the steps `iteration_boundary`, `llm_request`, `llm_response`, `budget_spent`, `exit`, and
   `skein ledger verify` reports both `ok`.
...
**Acceptance:**
1. **Given** `skein acp-agent --base-url http://192.168.1.10:11434/v1`, **When** it is invoked,
...
2. **Given** the same resolution rule, **When** `--base-url` is absent, **Then** the endpoint comes
   from `$SKEIN_MODEL_BASE_URL`, else `http://localhost:11434/v1` — identical to `skein chat`,
...
**Acceptance:**
1. **Given** `skein acp-agent` with a valid endpoint and silo, **When** its stdin reaches EOF
...
**Acceptance:**
1. **Given** a `--root` that cannot hold a silo, **When** `skein acp-agent` starts, **Then** it
...
- **FR-001**: `skein acp-agent` MUST serve exactly the ACP surface slice 008 implemented —
...
- **FR-002**: The model MUST be reached through `skein_gateway::{LocalEndpoint, OpenAiCompatClient}`.
...
- **FR-003**: Base-URL resolution MUST be `--base-url` → `$SKEIN_MODEL_BASE_URL` →
  `http://localhost:11434/v1`, identical to `skein chat`, and MUST have exactly one implementation
  shared by both commands.
- **FR-004**: Each session's `SessionParts.ledger` MUST be `Silo::open(root, id)?.ledger()?` — a
  durable, silo-backed chain. `Ledger::new()` MUST NOT appear in `skein-cli`.
...
- **FR-006**: `skein-acp` MUST gain exactly two things: `SkeinAgent::serve_stdio` and a fallible
  factory bound. `serve` itself, `SessionParts`'s fields, `SkeinSession`, `project_updates`,
  `AcpPermissionTransport` and `CancellableModel` MUST be otherwise unchanged.
- **FR-007**: `agent_client_protocol` MUST NOT be named outside `crates/skein-acp` in product code.
  (`skein-cli`'s **test** binary names it deliberately, because the acceptance criterion is a real
  ACP client.)
...
- **FR-010**: `skein-core` MUST gain exactly one additive variant, `Protocol(String)`, and nothing
  else.
- **FR-011**: The tool path MUST stay deny-by-default: `ToolPolicy::new(vec![], vec![])` with an
  unreachable-by-construction `ToolTransport`, shared with `skein chat`.
- **FR-012**: No automated test may require a running Ollama or an installed editor.
- **FR-013**: `crates/skein-mcp`, `crates/skein-silo` and `crates/skein-gateway` MUST be unchanged.
...
- **SC-003**: The ledger assertions run in a **second process** (`skein ledger log|verify`), so
...
- **SC-005**: `git diff dev -- crates/skein-mcp/ crates/skein-silo/ crates/skein-gateway/ spikes/
  .github/ rust-toolchain.toml` is empty; `git diff dev -- crates/skein-core/` is one variant plus
...
- **SC-006**: Dependency drift measured per target and recorded: expected **zero new packages**
  ...
  only — `skein-acp → futures`, `skein-cli → skein-acp`, and `skein-cli` dev →
  `agent-client-protocol` + `futures`.
- **SC-007**: `skein acp-agent --help` succeeds and `skein chat --help` still lists the same flags it
  listed on `dev`.
- **SC-008**: The hand-run against a real editor and a real Ollama is recorded with its transcript
  and `skein ledger verify` output — or explicitly recorded as unobserved, with the reason.
...
- **`futures::executor::block_on` is a complete runtime for this path.** Measured, not assumed: a
  two-binary probe outside the repository ran the exact shape `skein-acp` uses — a handler that
