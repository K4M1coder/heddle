...
- **`@modelcontextprotocol/server-filesystem`** — the reference implementation, and a **Node
  package**. Depending on it puts a Node runtime and an out-of-process child on `heddle chat`'s
  critical path, and hands back at runtime the local-only guarantee `heddle-gateway` makes a property
...
- **Build it** — three tools over one canonicalized directory, ~150 lines against the official
  ...
  `crates/heddle-mcp/tests/rmcp_gateway.rs`'s `DownstreamServer` already proves works against the
...
`heddle-mcp` is where it belongs: it is the MCP client, `tools/list` is an MCP method, and
...
### D5 — `crates/heddle-connectors`, a new workspace member

*Rejected: putting the server in `heddle-mcp`.* That crate's product dependencies are deliberately
...
The cost is that `heddle-mcp`'s docstring invariant — *"the only crate in the product that names the
MCP protocol"* — becomes false and must be amended in both crates, to: **`heddle-mcp` is the only
crate naming MCP as a client; `heddle-connectors` is the only one naming it as a server.** That
...
`heddle chat` builds the connector on the main thread; `heddle acp-agent` builds it inside
`HeddleAgent::open`, which runs under `futures::executor::block_on` (see `serve_stdio`), not a tokio
context. Both safe. The hazard is restated in `LocalConnector`'s docstring anyway.

`HeddleAgent` requires `T: ToolTransport + Send + 'static`. `RunningService<RoleClient, ()>`'s fields
...
| tool | `ToolAccess` | `heddle chat` | `heddle acp-agent` |
|---|---|---|---|
...
- **`heddle chat` is non-interactive.** Constitution VI requires confirmation for destructive actions
...
- **`heddle acp-agent` has a human behind an editor.** `fs_write` goes in `approved` so the policy
...
- **T2 · RED→GREEN** `crates/heddle-connectors` created: manifest, `lib.rs` docstring carrying D5's
  amended MCP invariant, `FsRoot`. Containment tests in `crates/heddle-connectors/tests/fs_root.rs`.
...
- **T5 · RED→GREEN** The headline acceptance test in
  `crates/heddle-connectors/tests/governed_fs_run.rs`, with `heddle-gateway` as a dev-dependency.
...
- **T7 · RED→GREEN** Redaction composition: a secret in a file's **contents**, scrubbed from the
  chain.
- **T8 · RED→GREEN** `heddle-cli`: `wiring::ToolArgs`, `ConfiguredTools`, the two policies,
  `--fs-root` on both commands, resolution ordered per D6.
- **T9 · RED→GREEN** CLI acceptance tests against the real binary.
- **T10** `#[ignore]`d live test, gated on `HEDDLE_LIVE_MODEL`, following `openai_compat.rs`'s
  precedent.
...
`crates/heddle-connectors` automatically — the workspace is `members = ["crates/*"]` and the
...
| **`LocalConnector` unexpectedly not `Send`**, so `heddle acp-agent` will not compile. | `RunningService`'s fields were checked and are all `Send`. Fallback: land `--fs-root` on `chat` only and split the ACP wiring. Do **not** reach for `Arc<Mutex<…>>`. |
