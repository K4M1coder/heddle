# Tasks: `heddle-cli` — the reference CLI client (v0 slice)

**Spec:** `specs/011-heddle-cli/spec.md` · TDD (red→green), product code in `crates/heddle-cli` plus
one additive method in `crates/heddle-core`, branch `011-heddle-cli` cut from `dev` after slice 010
merged.
...
- III Test-First ✅ (T1 pins the `clap` surface against the vendored source **and** a compiled
  ...
  IV Inverted coupling ✅ (`heddle-cli` has **no `lib` target**, so nothing can depend on the
...
- VIII Loop discipline ✅ (`LoopController`, `ProgressProbe` and `NativeLoop` untouched; v0 runs
  no loop)
- Cross-platform ✅ (no `#[cfg]` in the new crate; `#[command(bin_name = "heddle")]` makes usage
...
## Tasks
- [x] **T0** `specs/011-heddle-cli/{spec.md,plan.md,tasks.md}`; branch `011-heddle-cli` cut from
...
- [x] **T3** RED — `ledger_runs_lists_run_ids_in_first_append_order` in
      `crates/heddle-core/tests/core.rs` against the not-yet-existing `Ledger::runs()`
- [x] **T4** GREEN — `Ledger::runs()` in `crates/heddle-core/src/ledger.rs`
- [x] **T5** RED — `crates/heddle-cli/tests/cli_ledger.rs` (e1..e7) against a `fn main() {}` stub
- [x] **T6** GREEN — `src/main.rs` + `src/ledger.rs`
- [x] **T7** RED — `crates/heddle-cli/tests/cli_secret.rs` (e8..e9)
- [x] **T8** GREEN — `src/secret.rs`
- [x] **T9** gates: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D
      warnings`, `cargo test --workspace`, `cargo build --workspace` + run `heddle --help`
- [x] **T10** control diff: `git diff dev` empty on `crates/heddle-mcp/`, `crates/heddle-acp/`,
      `crates/heddle-silo/`, `spikes/`, `.github/` and `rust-toolchain.toml`
...
- [x] **T12** close out: correct `docs/DEVELOPMENT.md`'s two stale lines, tick the two stale
      bullets in `specs/003-heddle-core-foundation/tasks.md` and the first of spec 010's "Next
...
- **`env!("CARGO_BIN_EXE_heddle")` resolves to the built binary** from a `tests/*.rs` of the same
...
- **The exact derive shape used in T6/T8 was compiled and run.** `heddle secret set keychain://a/b
...
- **`#[command(bin_name = "heddle")]` is required.** Without it, `heddle ledger` renders
  `Usage: heddle.exe ledger <COMMAND>` on Windows and `Usage: heddle ledger …` elsewhere — measured
...
`cargo test --workspace` on `011-heddle-cli` @ `76450ed` (identical to `dev`), working tree clean,
2026-09-03, before any edit: **71 passing**, 0 failed, 0 ignored — `heddle-acp/tests/acp_session.rs`
13, `heddle-core/tests/core.rs` 12, `tests/native_loop.rs` 18, `tests/tool_gateway.rs` 9,
`heddle-mcp/tests/rmcp_gateway.rs` 7, `heddle-silo/tests/silo_ledger.rs` 7, `tests/silo_secret.rs` 5.
...
- **T3** `cargo test -p heddle-core --test core`, 2026-09-03:
  - `error[E0599]: no method named runs found for struct Ledger in the current scope`
    (`crates\heddle-core	ests\core.rs:285:13`)
  - `error: could not compile heddle-core (test "core") due to 1 previous error`
- **T5** `cargo test -p heddle-cli --test cli_ledger` against a `fn main() {}` stub, 2026-09-03:
...
- **T7** `cargo test -p heddle-cli --test cli_secret`, 2026-09-03: **1 failed, 1 passed.**
  - `e8_secret_set_then_delete_round_trips_without_printing_the_value` — the real red:
    `error: secret: keychain://heddle-cli-test-<pid>-0/cli: not implemented`, from the placeholder
  ...
  - `e9_secret_set_has_no_value_flag_and_refuses_an_empty_secret` **passed before its product code
    ...
    pinned), and the placeholder's `HeddleError::Secret` happened to produce the `secret:` prefix
...
- **82, not the advisory plan's 81.** The extra test is
  `e10_the_silo_root_falls_back_to_heddle_root_and_is_required`, which covers FR-005: `--root`
  wins, `$HEDDLE_ROOT` is the fallback, and neither is a loud exit 1 rather than a silent default.
  The advisory plan enumerated nine CLI tests and did not include one, but it also specified the
  `--root` → `$HEDDLE_ROOT` → error precedence as product behaviour in its step 7. Shipping a
...
- `cargo build --workspace` — clean; `target/debug/heddle.exe` exists (7.3 MB) and runs.
  `heddle --help` prints `Usage: heddle <COMMAND>` with the `ledger` and `secret` groups, and
  `heddle --version` prints `heddle 0.0.0` (SC-002). **`Usage: heddle`, not `heddle.exe`** — the
...
- The two `cli_secret` tests ran against the **real** Windows Credential Manager under service
  names unique per process and per test (`heddle-cli-test-<pid>-<n>`), each removed by a `Drop`
  guard. `cmdkey /list` afterwards matches nothing containing `heddle`, so the suite leaves the
...
harness has no PTY. It was exercised by hand instead — `Start-Process heddle.exe secret set
keychain://manual/probe` with a console stdin and only the output streams redirected:

```
EXITCODE=1
error: secret: refusing to read a secret from a terminal: pipe it instead, e.g. `printf %s "$TOKEN" | heddle secret set <REFERENCE>`
```
...
baseline tests — are live controls run against this slice's `heddle-core`.
...
`git diff dev --stat -- crates/heddle-core/` is `src/ledger.rs | 17 +++` and `tests/core.rs |
...
the three spec files, the six files of the new crate, the two `heddle-core` files, and the one root
...
- **`clap` adds exactly five external crates, and the same five on every target** —
  ...
  plus the new `heddle-cli` workspace member itself. `cargo tree -e normal,build,dev [--target …]`:

  | Target | before | after | added |
  |---|---|---|---|
  | `x86_64-pc-windows-msvc` (host) | 135 | 141 | the five above + `heddle-cli` |
  | `x86_64-unknown-linux-gnu` | 134 | 140 | the five above + `heddle-cli` |
  | `aarch64-apple-darwin` | 136 | 142 | the five above + `heddle-cli` |
...
- **`heddle-cli` has four direct dependencies** — `heddle-core`, `heddle-silo`, `clap`, `serde_json`
  — and two dev-dependencies, `tempfile` and `rusqlite`, both already in
  `[workspace.dependencies]`. It depends on **neither** `heddle-acp` nor `heddle-mcp`: there is
...
- [x] **`heddle chat`, together with the real `ModelClient`** (BMAD Story 1.4) — landed in slice
      012 (`specs/012-model-gateway/`) as `crates/heddle-gateway` plus `heddle-cli/src/chat.rs`. The
      deferral's estimate held: with a real client in hand, the chat surface did cost about one
      file. **`heddle acp-agent` is still open**, and is carried on slice 012's own "Next slice"
      list — it needs a stdio `ConnectTo<Agent>`, a `tokio` runtime inside `heddle-cli` and a
...
- [ ] `heddle ledger replay|revert|branch` — once `Ledger` has `replay(from)`, `revert(to)` and
...
- [ ] `--json` output, for the second consumer that needs something other than tab-separated
      columns
- [ ] a config file holding `SecretRef`s and a default silo root, so `--root`/`$HEDDLE_ROOT` is not
