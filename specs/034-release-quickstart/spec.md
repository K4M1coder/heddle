...
or `--help`, invent a `SKEIN_ROOT`, and independently work out that their Ollama model needs tool
support. Failure at any of those points is silent or cryptic, and the last one is not a failure at
all — it is a plausible answer with nothing of Skein behind it.
...
`dist/skein-0.1.0-windows-x64/` and its zip — 13 MB as a folder, 5.2 MB zipped — holding
`skein.exe`, the same `quickstart.ps1`, `QUICKSTART.md`, `README.md` and `LICENSE`. Copy the folder
to a share or send the zip. Nothing else is required of the recipient but PowerShell 7 and a local
provider.

**`skein --version` says `0.1.0`.** It said `0.0.0` — cargo's never-published placeholder — which is
...
1. **The check that carries this slice is the tool-capability check, and it exists because the
   ...
   Skein works when it did not run. Tool capability is therefore read off Ollama's **native**
   `/api/tags`, which reports a `capabilities` array, and not the OpenAI-compatible `/v1/models`
   route Skein itself uses, which does not.
...
- **FR-002** `skein --version` MUST print `skein 0.1.0`, with the version declared once on
...
- **FR-004** One script MUST serve both placements — a source checkout and a bundle beside
  `skein.exe` — distinguishing them by probing for the workspace manifest **and** `crates/`, so a
...
- **FR-012** The demo MUST be read-only and MUST NOT use `--allow-run`. `QUICKSTART.md` MUST name
  `--allow-run`, what it grants, that it is Windows-only in v0, and `skein sandbox list|prune` as
  its undo.
...
- **FR-016** The documentation MUST state that this path is verified on Windows only, that
  `skein.exe` is **unsigned**, and that `cargo install --path` is documented but not exercised.
...
| 4 | a staleness check over `src/**` before building | measured: an already-current `cargo build --release -p skein-cli` returns in 0.17–0.23 s. Cargo already makes that decision, faster and more correctly than a timestamp comparison would |
...
| 7 | leave `0.0.0` and name the bundle by short SHA | fewer edits, but it puts `skein-v0-ae3a5a9` on the folder while the binary inside insists it is `0.0.0`. The confusion is moved somewhere less visible, not removed |
...
| 12 | `cargo install --path crates/skein-cli` as the primary path | good for a colleague who already has Rust and a C compiler, so it is documented — but it reimposes the toolchain and the cold build the bundle exists to remove, which is most of the onboarding gap |
