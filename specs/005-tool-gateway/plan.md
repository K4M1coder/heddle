...
## Summary
Give the product its first enforcement point for tool calls. `skein-core` gains one module,
...
workspace member, `skein-mcp`, holds `RmcpToolTransport`, the only place that names `rmcp` or
...
**Primary Dependencies**: `rmcp` 2.2 + `tokio` 1 in `skein-mcp` only; `schemars`/`serde` as
`skein-mcp` dev-dependencies for the embedded fixture server. `skein-core` unchanged.
**Storage**: the existing in-memory `Ledger` (durable SQLite-backed silo still deferred)
**Testing**: `cargo test`; a hand-rolled `CountingTransport` double in `skein-core`, and a
live in-process rmcp server over `tokio::io::duplex` in `skein-mcp`. No mocking crate.
...
- **II. Local-first / silo isolation**: ✅ no network — the rmcp tests speak to an in-process
  duplex, never a socket or a spawned process. `skein-core` gains no dependency.
...
- **IV. Inverted coupling**: ✅ `ToolTransport` is the seam; `skein-core` physically cannot
...
crates/skein-core/
  ...
  src/error.rs            # +2 variants: SkeinError::ToolDenied, SkeinError::Tool
  tests/tool_gateway.rs   # new — 6 acceptance tests + CountingTransport
crates/skein-mcp/         # new workspace member (crates/* already globs it in)
  Cargo.toml
...
```
**Structure Decision**: the governed path lives in `skein-core` and the protocol adapter in a
second crate. A `#[cfg(feature = "mcp")]` gate inside `skein-core` was rejected: `core.yml`
