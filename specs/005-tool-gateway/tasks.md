...
`crates/heddle-core` and the new `crates/heddle-mcp`, branch `005-tool-gateway` cut from `dev`.

## Constitution Check (ADR-0004 D1 solo-v0 bar)
- I Headless core ✅ (library API; no UI, no bin) · II Local-first ✅ (no network; the rmcp
  tests speak to an in-process duplex, and `heddle-core` gains no dependency)
- III Test-First ✅ (T3 red before T4 green; T6 red before T7 green) · IV Inverted coupling ✅
  (`ToolTransport` is the seam; `heddle-core`'s `Cargo.toml` cannot name `rmcp`)
...
- [x] **T2** `HeddleError::ToolDenied` and `HeddleError::Tool` (FR-002/FR-006)
- [x] **T3** RED — `crates/heddle-core/tests/tool_gateway.rs` with `CountingTransport` and all 6
...
- [x] **T5** `crates/heddle-mcp` skeleton + root `[workspace.dependencies]` entries (FR-007)
- [x] **T6** RED — `crates/heddle-mcp/tests/rmcp_gateway.rs` with the live embedded rmcp server
...
- [x] **T8** `fmt --check`, `clippy --workspace --all-targets -D warnings`,
      ...
      gateway + 5 rmcp), 2026-09-03; `git diff` on `crates/heddle-core/Cargo.toml` empty
      (SC-001/SC-003)
...
- **T3** `cargo test -p heddle-core`:
  `error[E0432]: unresolved imports heddle_core::replay_tool_calls, heddle_core::Redactor,`
  `heddle_core::ToolCall, heddle_core::ToolGateway, heddle_core::ToolOutcome,`
  `heddle_core::ToolPolicy, heddle_core::ToolTransport` —
  `error: could not compile heddle-core (test "tool_gateway")`.
- **T6** `cargo test -p heddle-mcp`:
  `error[E0432]: unresolved import heddle_mcp::RmcpToolTransport` —
  `no RmcpToolTransport in the root` —
  `error: could not compile heddle-mcp (test "rmcp_gateway")`.
