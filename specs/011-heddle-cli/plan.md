# Implementation Plan: `heddle-cli` — the reference CLI client (v0 slice)

**Branch**: `011-heddle-cli` | **Date**: 2026-09-03 | **Spec**: `specs/011-heddle-cli/spec.md`

## Summary
A new workspace member `crates/heddle-cli` with **`[[bin]] name = "heddle"`** and no `lib` target —
the first executable in the repository. It exposes two command groups, both end-to-end real:

```
heddle ledger log|show|verify --silo <ID> [--root <PATH>] …
heddle secret set|delete <REFERENCE>
```
...
`runs()` a person who does not already know an id cannot use `heddle ledger log` at all. "Everything
...
`crates/heddle-silo` gets **no** API change; `heddle-mcp` and `heddle-acp` are untouched.
...
`heddle-cli`. Plus `heddle-core`, `heddle-silo` and `serde_json` (for `FR-012`'s serde kind name).
**Not** `heddle-acp` or `heddle-mcp`: there is nothing in either that v0 can call without a model,
...
**Testing**: `cargo test`; one seam test in `heddle-core`, nine **process** tests in `heddle-cli`
driving the real binary via `env!("CARGO_BIN_EXE_heddle")`. Dev-dependencies `tempfile` (silo roots)
...
`crates/heddle-mcp/`, `crates/heddle-acp/`, `crates/heddle-silo/` byte-identical to `dev`
...
`Command::new(env!("CARGO_BIN_EXE_heddle"))` reaches the real binary with zero dependencies, and
...
`heddle secret set <REFERENCE>` reads stdin to EOF and strips at most one trailing newline. There is
...
- **IV. Inverted coupling**: ✅ `heddle-cli` has no `lib` target, so nothing can depend on it. It
  depends on `heddle-core` and `heddle-silo` and on neither of the two protocol crates.
...
- **Cross-platform**: ✅ no `#[cfg]` anywhere in the new crate. `#[command(bin_name = "heddle")]`
  makes usage text identical on all three OSes (without it clap renders `Usage: heddle.exe …` on
...
docs/DEVELOPMENT.md               # two corrected lines (test tooling; `heddle secret set`)
crates/heddle-core/
  src/ledger.rs                   # +Ledger::runs()
  tests/core.rs                   # +1 test
crates/heddle-cli/                 # NEW
  Cargo.toml                      # [[bin]] name = "heddle"; no lib target
...
```
**Structure Decision**: `crates/heddle-mcp/`, `crates/heddle-acp/` and `crates/heddle-silo/` are
...
| **An unknown `--silo` leaves an empty directory behind** | `Silo::open`'s id validation is the security-relevant part and must not be re-implemented in the CLI, and `Silo::open` `create_dir_all`s. The ledger file guard turns the wart into a loud exit 1 rather than a silently empty log. | `Silo::open_existing()` in `heddle-silo`: a second crate's API change for a cosmetic gain, and this slice's control argument depends on `heddle-silo` being byte-identical to `dev`. |
...
| **`heddle ledger show` can print a secret that reached an `LlmRequest`/`LlmResponse` payload** | It is a pre-existing property of the Ledger (`NativeLoop::run` appends model I/O raw), and Principle I says the CLI exposes what the API exposes. | Redacting in the CLI: would make the CLI's view *differ* from the API's, hiding a real gap behind the one surface an auditor looks at. The fix belongs on the model-I/O path; it is on the next-slice list. |
