...
ADR-0003 decision 2 and ADR-0004 D3 (**adopt ACP as Heddle's client↔core boundary**) ·
design §4.2/§4.3/§4.11/§4.14.

Four merged slices give `heddle-core` a working, governed, tested agentic loop — `NativeLoop`
...
binary can drive it.** The only "client" today is `crates/heddle-core/tests/native_loop.rs`.

Constitution Principle I cannot begin to be satisfied until the core is reachable by a real
client, and ADR-0003 already decided what that boundary is: ACP, not a bespoke Heddle wire
...
governed loop end to end, in a new crate `heddle-acp` that is the only crate in the product to
name ACP. `heddle-core` and `heddle-mcp` are unchanged.
...
**Acceptance:**
1. **Given** a real `agent-client-protocol` client connected to `heddle-acp` over a byte-stream
...
## Requirements
- **FR-001**: A new workspace crate `heddle-acp` MUST be the only crate that names ACP; the
  dependency direction is `heddle-acp` to `heddle-core`, never the reverse.
- **FR-002**: **No file under `crates/heddle-core/` or `crates/heddle-mcp/` may change.** ACP is
...
- **FR-005**: A client's decline MUST surface as `HeddleError::ToolDenied`, so the existing
...
- **FR-007**: One ACP `SessionId` owns N Heddle runs, one per `session/prompt`, with run ids
...
- **SC-002**: The end-to-end acceptance runs against a **real** ACP client and a **real** ACP
  ...
  for the protocol.
- **SC-003**: `git diff dev -- crates/heddle-core/ crates/heddle-mcp/` is empty.
...
- **Each `session/prompt` is an independent Heddle run with no conversation history.**
