...
**write-through store** behind a trait `skein-core` owns:
...
["bundled"] }`, confined to `crates/skein-silo/src/ledger_store.rs`. Dev-only: `tempfile`.
`skein-core` keeps exactly four dependencies.
**Storage**: SQLite, one file per silo, rollback journal, `PRAGMA synchronous = FULL`
**Testing**: `cargo test`; three seam tests in `skein-core` against a `VecStore`/failing-store
double, seven file-backed tests in `skein-silo` over a `tempfile::TempDir` root
**Target Platform**: Windows + macOS + Linux
**Project Type**: library (four workspace members)
**Performance Goals**: N/A
**Constraints**: `skein-core` may not name a database; `crates/skein-mcp/` unchanged; no network
...
- **IV. Inverted coupling**: ✅ `LedgerStore` is the seam. `rusqlite` is named in exactly one
  module of one crate and never in `skein-core`, whose direct dependency list stays four.
...
crates/skein-core/
  src/error.rs                    # +SkeinError::Storage
...
```
**Structure Decision**: `crates/skein-mcp/` is byte-identical to `dev`, so spec 005's suite
