...
`git status --short` in `D:\claudecode\heddle` was empty when this plan was written and is empty now.
...
its containment properties are proven hermetically in `crates/heddle-sandbox/tests/escape.rs`, and its
...
1. **`crates/heddle-connectors/src/run.rs` — `resolve_exe(root: &FsRoot, command: &str)`.** The request
   located this in `crates/heddle-sandbox/src/launch.rs`; it is not there. `launch.rs` takes an
   already-resolved absolute `exe: &Path`. Resolution lives in `heddle-connectors`, and that boundary is
...
2. **`crates/heddle-sandbox/src/profile.rs` — `create(root)` and `unsafe fn grant(root, sid)`.** The
...
3. **`crates/heddle-sandbox/src/lib.rs`** — `Sandbox { root: PathBuf, sid: String }`,
...
4. **`crates/heddle-sandbox/src/launch.rs` — `environment_block()`.** Five variables, sorted
...
5. **`crates/heddle-connectors/src/server.rs`** — `pub enum RunAccess { Denied, Allowed }`
...
6. **`crates/heddle-connectors/src/connector.rs`** — `local_connector_with_run(root, run)`.
7. **`crates/heddle-cli/src/wiring.rs`** — `ToolArgs { fs_root: Option<PathBuf> }` (**no environment
...
8. **`crates/heddle-cli/src/acp.rs` — `serve`.** `run.resolve()?` is called before `Silo::open`, then
...
9. **Environment conventions.** `$HEDDLE_ROOT` backs `--root` (silo) and `$HEDDLE_MODEL_BASE_URL` backs
...
10. **Call sites that will move.** `Sandbox::create` in `crates/heddle-sandbox/tests/{profile.rs,
    launch.rs,escape.rs}`; `RunAccess::{Allowed,Denied}` in `crates/heddle-connectors/tests/{connector.rs,
    run_server.rs,governed_proc_run.rs}`; `--allow-run` in `crates/heddle-cli/tests/cli_acp_agent.rs`
    (the help-text test `acp_agent_documents_the_allow_run_flag_and_chat_does_not`, and the
    allow/reject governed pair).
11. **`RmcpToolTransport::list` (`crates/heddle-mcp/src/lib.rs`, `fn spec_of(tool: &Tool)`)** maps only
...
15. **`C:\Program Files\nodejs` does *not* inherit `C:\Program Files`' AppContainer ACEs** — its DACL is
    ...
    it needs `WRITE_DAC`, which a non-elevated heddle does not have. A real failure mode a live
...
### D1 — The flag is `--run-dir <PATH>`, repeatable, on `heddle acp-agent` only, with no environment fallback

Added to `RunArgs` in `crates/heddle-cli/src/wiring.rs`, beside `--allow-run`:
...
`heddle chat` is untouched — `chat_policy`'s existing docstring already records why a mutating tool that
...
**Rejected — an environment fallback (`$HEDDLE_RUN_DIRS`).** The request asked which convention fits.
Verified fact 9: `$HEDDLE_ROOT` and `$HEDDLE_MODEL_BASE_URL` exist, but **no** path-scoped
...
The cost, stated: `RunAccess` loses `Copy`, so `crates/heddle-cli/src/acp.rs`'s session factory clones it
...
### D3 — `RunDirs` is a validated newtype in `crates/heddle-connectors/src/fs.rs`, beside `FsRoot`

```rust
pub struct RunDirs(Vec<PathBuf>);
impl RunDirs {
    pub fn none() -> RunDirs;
    pub fn new(paths: &[PathBuf]) -> heddle_core::Result<RunDirs>;  // canonicalize each, refuse a non-directory
...
sees one spelling, exactly as `FsRoot` does; `win32_path` in `heddle-sandbox` already strips the
...
`crates/heddle-connectors/src/lib.rs` beside `RunAccess`.
...
`crates/heddle-sandbox/src/profile.rs`'s `grant` gains a mask parameter:
...
`crates/heddle-mcp/src/lib.rs` maps only name, description and parameters.
...
heddle once, or name a directory you own.
...
**T2 — Types and signatures, no new behaviour.** `RunDirs` in `crates/heddle-connectors/src/fs.rs` (real
...
`crates/heddle-sandbox/tests/profile.rs`, beside
...
`crates/heddle-sandbox/tests/launch.rs`. Fixture: one `TempDir` root, a second `TempDir` "toolbin",
...
`crates/heddle-sandbox/tests/escape.rs`, in that file's positive-control style. A sandboxed
...
`crates/heddle-connectors/tests/run_server.rs`, extending its `Fixture`/`run` helpers with a run directory:
...
**T7 — RED→GREEN: the advertisement.** Test in `crates/heddle-connectors/tests/connector.rs` beside
...
**T8 — RED→GREEN: the CLI.** Tests in `crates/heddle-cli/tests/cli_acp_agent.rs`, in the style of
`acp_agent_documents_the_allow_run_flag_and_chat_does_not`:
- `heddle acp-agent --help` documents `--run-dir`; `heddle chat --help` does not;
...
`crates/heddle-connectors/tests/governed_proc_run.rs` with a second `#[ignore]`d live test that configures a
run directory from `$HEDDLE_LIVE_RUN_DIR` and prompts for a real toolchain binary (below). Add `heddle-silo`
to `crates/heddle-connectors/Cargo.toml`'s `[dev-dependencies]` so that test can persist to a real silo when
`$HEDDLE_LIVE_SILO_ROOT` is set — `Silo::open(root, "live020")?.ledger()?` returns the same
`heddle_core::Ledger` the in-memory path uses, verified in `crates/heddle-silo/src/lib.rs` — and fall back to
...
- Control diff empty for `crates/heddle-silo/`, `crates/heddle-core/`, `crates/heddle-gateway/`,
  `crates/heddle-mcp/`, `spikes/`, `.github/`, `rust-toolchain.toml`. **One stated exception:**
  `crates/heddle-connectors/Cargo.toml` gains a single `[dev-dependencies]` entry (`heddle-silo`) for T9's
  optional persistence. No product dependency is added anywhere; `heddle-sandbox`'s dependency set is
  untouched.
...
|---|---|---|
| `a_run_dir_is_granted_read_and_execute_and_the_root_is_not` | `heddle-sandbox/tests/profile.rs` | Real `GetNamedSecurityInfoW` read-back, masks normalised through `MapGenericMask`: the run dir's ACEs carry `FILE_READ_DATA` and `FILE_EXECUTE` and **not** `FILE_WRITE_DATA` / `FILE_APPEND_DATA` / `WRITE_DAC`; the fs-root's **do** carry `FILE_WRITE_DATA`. Narrower, measured against the object's own descriptor rather than asserted about intent. |
| `a_binary_in_an_allowlisted_run_dir_executes_and_its_stdout_comes_back` | `heddle-sandbox/tests/launch.rs` | A real ACE grant + a real `CreateProcessW` + real captured stdout, on a directory carrying no `ALL APPLICATION PACKAGES` ACE. |
| `a_sandboxed_process_cannot_write_into_a_run_dir` | `heddle-sandbox/tests/escape.rs` | Narrowness as an **effect**: the copy lands in the fs-root (control) and not in the run dir. |
| `a_bare_name_in_an_allowlisted_run_dir_resolves_and_runs` | `heddle-connectors/tests/run_server.rs` | The resolution rule at the tool's own level. |
| `a_bare_name_in_a_directory_that_was_not_named_names_every_place_it_looked` | `heddle-connectors/tests/run_server.rs` | The allowlist has not become a de facto `%PATH%`, and the refusal enumerates System32, `%SystemRoot%` and every named directory. |
| `system32_still_wins_over_a_run_dir_that_shadows_it` | `heddle-connectors/tests/run_server.rs` | D5's append-not-prepend order, proven by a run that can only succeed if System32 was searched first. |
| `the_advertised_description_names_the_allowlisted_directories` | `heddle-connectors/tests/connector.rs` | D9 — and, with no run dir, that the advertisement keeps everything slice 019 pinned. |
| `acp_agent_documents_the_run_dir_flag_and_chat_does_not` | `heddle-cli/tests/cli_acp_agent.rs` | The flag exists on exactly one subcommand. |
| `run_dir_without_allow_run_is_an_exit_code_naming_both_flags` | `heddle-cli/tests/cli_acp_agent.rs` | Deny-by-default at the operator boundary, against the real binary. |
| `a_run_dir_that_is_not_a_directory_is_a_loud_refusal` | `heddle-connectors/tests/fs_root.rs` | `RunDirs::new`'s validation, in the file that already owns `FsRoot::new`'s. |
| `a_live_model_runs_a_real_toolchain_binary` (`#[ignore]`) | `heddle-connectors/tests/governed_proc_run.rs` | Repeatable hand-verification, `governed_fs_run.rs`'s established pattern. |
...
- `node --version` is the documented **fallback**: it also survives that environment, but
  ...
  elevated heddle to grant. If the operator prefers node, that is the cost — stated here rather than met as
  `ERROR_ACCESS_DENIED`;
...
AppContainer. The `heddle ledger show` output for that step is the evidence pasted into `tasks.md`.
...
**Blast radius.** `crates/heddle-sandbox` (`lib.rs`, `profile.rs`, `launch.rs`), `crates/heddle-connectors`
(`fs.rs`, `run.rs`, `server.rs`, `lib.rs`, `Cargo.toml` dev-deps), `crates/heddle-cli` (`wiring.rs`,
`acp.rs`). `heddle-core`, `heddle-silo`, `heddle-gateway`, `heddle-mcp` and `spikes/` are untouched. No new
product dependency, and no new `unsafe` block outside `heddle-sandbox` — the mask parameter reuses `grant`'s
existing one.
...
- **Profile and ACE cleanup** (`heddle sandbox prune`, `DeleteAppContainerProfile`). A separately named
