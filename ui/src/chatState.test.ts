...
// is the **wire** shape `heddle acp-agent` actually puts on the ACP connection —
...
describe("applyUpdate - agent_message_chunk", () => {
  ...
  it("keeps one entry per chunk rather than merging them", () => {
    // `heddle-acp` emits exactly one chunk per `LlmResponse` step, so two
...
describe("the batch a single turn delivers", () => {
  it("keeps pending set until the prompt itself resolves", () => {
    // Correction 3: `heddle-acp` sends every update for a run in one burst
...
describe("send and cancel guards", () => {
  it("refuses an empty or whitespace-only message", () => {
    // Mirrors `crates/heddle-cli/src/chat.rs`'s refusal of an empty prompt: the
