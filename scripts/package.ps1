# Heddle — assemble the distributable Windows bundle and its zip.
...
$bundleName = "heddle-$version-windows-x64"
$distDir = Join-Path $repoRoot 'dist'
$bundleDir = Join-Path $distDir $bundleName
$zipPath = Join-Path $distDir "$bundleName.zip"

Step "Release build"
# --workspace and not just -p heddle-cli: packaging must not be able to outrun
...
# heddle.pdb is deliberately absent: 7 MiB of separate debug symbols that serve
# nobody without the matching source tree, on a 13 MiB payload.
$payload = @(
  (Join-Path $repoRoot 'target/release/heddle.exe'),
