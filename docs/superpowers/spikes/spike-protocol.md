...
**Question:** which execution tier lets Heddle own the loop — (A) native Rust loop (`rmcp` + OpenAI-compat client), (B) embedded `goose-sdk` crates, (C) ACP worker (drive goosed/OpenCode via `agent-client-protocol`)?
...
**Question:** can one Archon-style YAML workflow map losslessly onto Heddle's canonical graph (nodes agent/tool/approval/cond/parallel/loop)?
...
**Question:** can Heddle proxy MCP servers (1 local stdio + 1 remote OAuth) through policy/approval/redaction/Ledger capture using `rmcp`?
...
**Exit:** on each OS (VM/CI): bootstrap script → build → `heddle`-equivalent binary runs a local-model turn with network disabled; failures documented per OS.
