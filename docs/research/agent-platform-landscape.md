...
**Project:** Skein — independent open-source project

## Purpose

This report evaluates existing coding agents, personal assistants, workflow engines, chat platforms, and long-context research to decide what Skein should adopt, adapt, learn from, or implement internally.
...
- **Adopt** — use as a replaceable component behind a Skein-owned contract.
...
No external product may own Skein's canonical artifacts, authorization decisions, workflow state, Ledger, or completion verdict.

## Capability matrix

| Product | Strongest reusable ideas | Strategy for Skein | Main reason |
|---|---|---|---|
...
| OpenCode | Open server/CLI/TUI architecture, multi-provider agent, custom agents, SDK-like surfaces | **Adapt selectively + optional Worker** | MIT and architecturally useful; TypeScript/Bun runtime must not own Skein state |
...
| Hermes Agent | Procedural memory, self-created skills, messaging gateway, terminal backends, trajectory compression, local browser | **Inspire + optional Worker** | Rich assistant behavior; Python runtime and its learning loop are not Skein's policy boundary |
...
| OpenClaw | Always-on local gateway, channels, routing, cron, device nodes, pairing | **Inspire only** | Valuable assistant UX, but its single-trusted-operator model and security history do not satisfy Skein team/RBAC requirements |
| LibreChat | Multi-provider chat, MCP UX, per-user credentials, LDAP/OIDC, artifacts, RAG and multi-user patterns | **Inspire; optionally reuse isolated UI packages after review** | MIT and feature-rich, but its Mongo/Node/Python deployment is too heavy as Skein's core |
...
| LiteLLM | Provider normalization, routing, budgets, fallback, OpenAI-compatible gateway | **Adopt as replaceable adapter initially** | Avoids reimplementing provider breadth; Skein retains its own capability model |

## Architecture conclusion

Skein should be a **modular monolith with optional sidecars and workers**:
...
The proprietary or third-party worker is never the control plane. Skein owns:
...
## Canonical contracts Skein must own
...
Skein therefore treats 1M context as overflow capacity, not working memory.

### Context operating model

For each model call, Skein builds a reproducible `ContextManifest`:
...
Skein should expect to implement roughly the differentiating control plane while reusing commodity infrastructure:
...
2. **Workflow reuse:** prototype one Archon-compatible YAML workflow and map it losslessly to Skein's canonical workflow graph.
