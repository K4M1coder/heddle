...
**The window has no capability of its own.** It does not talk to a model, does not run the loop, does not own a tool gateway and does not touch a silo. It spawns `skein acp-agent` as a child process and speaks the Agent Client Protocol to it — the same protocol, over the same stdio transport, that an ACP-speaking editor uses, and that `crates/skein-cli/tests/cli_acp_agent.rs` already drives against the same binary. Every button below is one ACP call the CLI already serves (Constitution I).
...
| **A local model provider** | `--with-ollama` during bootstrap, or anything OpenAI-compatible on loopback. `skein-gateway` compiles `ureq` with no TLS backend, so **loopback `http://` is the only thing it can reach** (Constitution II). |
...
v0 has no settings screen and no config file. It reads the environment, and it **refuses to start naming the variable** when a required one is missing — for the same reason `skein --root` refuses to guess a silo root: guessing would put an agent's journal somewhere the operator did not name.

| Variable | Required | Becomes | Notes |
|---|---|---|---|
| `SKEIN_ROOT` | **yes** | `--root` | The directory holding the silos. |
| `SKEIN_UI_MODEL` | **yes** | `--model` | No default, exactly as the CLI has none. |
| `SKEIN_UI_SILO` | no | `--silo` | Defaults to `ui`. |
| `SKEIN_MODEL_BASE_URL` | no | `--base-url` | Omitted entirely when unset, so the CLI's own default applies. |
| `SKEIN_UI_FS_ROOT` | no | `--fs-root` | **Absent means the session has no tools at all** — `crates/skein-cli/src/wiring.rs`'s "no root, no tools". |
| `SKEIN_UI_BIN` | no | — | Path to the `skein` binary. Defaults to the one beside the app's own executable. |
...
cargo build -p skein-ui --release
```

> `ui/dist` must exist before `cargo build -p skein-ui`: `tauri.conf.json`'s `frontendDist` points at it and `tauri-build` reads it at compile time. `npm run build` first, always. `.github/workflows/core.yml` does exactly that, in that order.

The window expects the `skein` binary beside its own executable. In a `cargo build` layout that is already true (`target/debug/skein` and `target/debug/skein-ui` are siblings); otherwise set `SKEIN_UI_BIN`.

---

## What each action does

Every row is one ACP method the `skein acp-agent` subcommand already serves. There is no fourth thing the window can do.

| UI action | Exact call it triggers |
|---|---|
| **App launch** | Spawns `skein acp-agent --root … --silo … --model … [--base-url …] [--fs-root …]`, then sends ACP `initialize`, then `session/new`. Returns the session id the chain records runs under (`skein-1`, `skein-1#1` for its first run). |
| **Send** | Tauri command `send_prompt(text)` → ACP `session/prompt` — the request `crates/skein-acp/src/lib.rs` answers by running the native loop. |
| **transcript update** (passive) | ACP `session/update` notifications, relayed **1:1 and untransformed** as the Tauri event `session-update`. The projection that produced them is `skein-acp`'s `project_updates`, reading the run's chain. The window is a view of that view and adds nothing to it (Constitution V). |
| **Cancel** | Tauri command `cancel_run()` → ACP `session/cancel` notification. |
| **Closing the window** | Closes the client's end of the pipe. `skein acp-agent` exits zero when its client disconnects, so nothing is killed and nothing leaks. |
...
1. Export `SKEIN_ROOT` and `SKEIN_UI_MODEL` (e.g. `llama3.1`), with a local provider running.
...
4. Send a second message. It runs on the same session — the chain records `skein-1#1` and `skein-1#2` under the same silo. Verify with:
   ```bash
   skein ledger log --root "$SKEIN_ROOT" --silo ui --run skein-1#2
   ```
...
6. Close the window, then confirm no `skein` process is left behind.
...
`SkeinAgent` supports many sessions; this window drives exactly one. No tabs, no session switcher.
