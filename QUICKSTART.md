...
Bypass` covers the case where it is still refused. `heddle.exe` is **not code-signed** either, so
...
2. Finds `heddle.exe` beside itself.
3. Asks your local model provider — Ollama on `http://localhost:11434/v1` unless you pass
   ...
   would see a plausible paragraph and conclude Heddle worked when nothing of Heddle ran.
...
5. Runs one turn: `heddle chat`, read-only, scoped to the current directory.
6. Prints the run's ledger — the `tool_call` / `approval` / `tool_result` triple is the agent
   actually reading a file through a governed tool — and then `heddle ledger verify`, which
...
Heddle talks to local providers over `http` only, on loopback. There is no TLS backend compiled into
the binary, so an `https://` base URL is refused rather than attempted — a build-time property, not
a setting.

## What it leaves behind

One directory: `%LOCALAPPDATA%\heddle\quickstart-demo`, holding the demo silo and its ledger. The
...
Nothing else. The demo turn is read-only by construction rather than by care: `heddle chat`'s tool
allowlist has no `fs_write` on it at all, because a non-interactive command has nobody to confirm a
destructive action to.

## Going further

`heddle --help` lists the whole CLI: `chat`, `acp-agent`, `ledger`, `secret`, `sandbox`.

The one capability the quickstart deliberately never exercises is **`--allow-run`** on
`heddle acp-agent`, which offers the sandboxed `proc_run` tool. It is worth knowing what it costs
...
- `heddle sandbox list` shows every profile and grant on the machine, and `heddle sandbox prune`
...
identically from a clone — it builds `heddle.exe` first and points the agent at the repo root.

`cargo install --path crates/heddle-cli` puts `heddle` on your `PATH` instead of in a folder. That
...
platforms in CI, but this onboarding path is not, and saying otherwise would be a guess. Heddle 0.1.0
