...
A first Phase-1 desktop slice now sits on top of that core: a Tauri window (`ui/`) drives the same `heddle acp-agent` an editor would, so every UI action is a call the CLI already serves — see [the UI guide](docs/UI.md) for how to run it and what each button sends. It has three screens. **Chat** sends ACP `session/prompt` and `session/cancel`. **Code** browses and reads the session's `--fs-root` through `list_directory`/`read_file`, which call the very `heddle_connectors::FsRoot` the agent's own `fs_list`/`fs_read` tools resolve every path through — so it reaches exactly what the session reaches, and it is read-only for now. **Settings** calls `session_settings()` to report what the running child was actually launched with: `fs` and `shell` from `--fs-root` and `--allow-run`, `git` from the same `is_git_repository` check the CLI uses to decide whether to offer the git tools, and Atlassian and Microsoft 365 truthfully as *not wired to a session* — because no `heddle` subcommand accepts a flag that enables either one yet. Editing files from the window, a tool-permission dialog and any connector-enablement UI are deliberately still out — see [`specs/041-tauri-code-view/spec.md`](specs/041-tauri-code-view/spec.md).
